...
export function loadSubmissions():OwnSubmission[]{
 ...
 try{return JSON.parse(readScoped(KEY)||"[]")}catch{return[]}
}
export function recordSubmission(entry:Omit<OwnSubmission,"id"|"at">):OwnSubmission{
 ...
 if(typeof window!=="undefined"){
  ...
  list.push(row);
  writeScoped(KEY,JSON.stringify(list.slice(-300)));
...
export function problemStatus(problemKey:string,extraSolvedId?:string):ProblemStatus{
 ...
 if(extraSolvedId){
  try{
   const mastery=JSON.parse(readScoped("algoyol-mastery")||"{}") as {evidence?:Record<string,number>};
