...
export function AlgoYolApp(){
 ...
 const screenRef=useRef<{view:View;roadmap:string;unit:string|null}>({view:"home",roadmap:"foundations",unit:null}),navDepth=useRef(0);
 ...
 const pushScreen=(patch:Partial<{view:View;roadmap:string;unit:string|null}>)=>{const next={...screenRef.current,...patch};applyScreen(next);window.history.pushState(next,"");navDepth.current++;window.scrollTo({top:0,behavior:"smooth"})};
 ...
 useEffect(()=>{window.history.replaceState(screenRef.current,"");const onPop=(e:PopStateEvent)=>{const st=e.state as {view:View;roadmap:string;unit:string|null}|null;applyScreen(st&&typeof st==="object"&&"view" in st?st:{view:"home",roadmap:"foundations",unit:null});navDepth.current=Math.max(0,navDepth.current-1)};window.addEventListener("popstate",onPop);return()=>window.removeEventListener("popstate",onPop)},[]); const swap=()=>{const n=lang==="uz"?"en":"uz";setLang(n);localStorage.setItem("algoyol-lang",n)};
