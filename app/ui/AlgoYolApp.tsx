...
import { FriendsScreen, ProblemSubmissions, SiteSubmissions, SubmissionsScreen } from "./social-ui";
...
type Lang="uz"|"en"; type View="home"|"roadmaps"|"roadmap"|"problems"|"problem"|"problem-submissions"|"duel"|"leaderboard"|"profile"|"auth"|"admin"|"placement"|"stats"|"users"|"messages"|"person"|"shop"|"friends"|"submissions"|"person-submissions"|"person-duels"|"person-friends"|"site-submissions"|"duel-history"|"playground"|"notfound";
...
export function AlgoYolApp(){
 ...
 const hydrateAccountState=async()=>{
  ...
  // The account's own verdicts decide which rows are green and which are red,
 ...
 useEffect(()=>{
  ...
  let live=true;
  /* Only a rejection signs you out. A reload cancels the requests the previous
     load had in flight, and treating that as "not signed in" is what threw a
     learner out of the site for pressing refresh twice quickly — the token was
     valid, the answer simply never came back. When we cannot ask, the last
     verified profile carries the session until the next check succeeds. */
  void verifyStoredSession().then(check=>{
   if(!live)return;
   if(check.status==="invalid"){clearSession();setScope(GUEST_SCOPE);setAuth({status:"guest"});return}
   const next=check.profile;
   if(!next){setAuth({status:"guest"});return}
...
const wasDone=!!data.solved[lesson]&&(data.quizScores[lesson]||0)>=70;data.solved={...data.solved,[lesson]:true};if(!wasDone&&(data.quizScores[lesson]||0)>=70)recordTopicDone();writeScoped("algoyol-roadmap-progress",JSON.stringify(data));removeScoped("algoyol-active-lesson");window.dispatchEvent(new Event("algoyol-progress"))},[verdict]);// eslint-disable-line react-hooks/exhaustive-deps
 ...
 const screenToPath=(s:Screen):string=>{
  ...
  if(s.view==="person-duels")return s.person?`/u/${s.person}/duels`:"/";
  if(s.view==="person-friends")return s.person?`/u/${s.person}/friends`:"/";
  if(s.view==="problem")return s.problem?`/problem/${s.problem}`:"/problems";
  if(s.view==="problem-submissions")return s.problem?`/problem/${s.problem}/submissions`:"/problems";
  return `/${s.view}`;
 };
 const pathToScreen=(path:string):Screen=>{
  ...
  if(parts[0]==="u"&&parts[1])return {...base,view:parts[2]==="submissions"?"person-submissions":parts[2]==="duels"?"person-duels":parts[2]==="friends"?"person-friends":"person",person:decodeURIComponent(parts[1])};
  if(parts[0]==="problem"&&parts[1]){
   ...
   if(found&&parts[2]&&parts[2]!=="submissions")return {...base,view:"notfound"};
   return found?{...base,view:parts[2]==="submissions"?"problem-submissions":"problem",problem:found.id}:{...base,view:"notfound"};
  }
 ...
 const applyScreen=(s:Screen)=>{
  ...
  if((s.view==="problem"||s.view==="problem-submissions")&&s.problem){
 ...
 useEffect(()=>{
  ...
  const channel=openDuelChannel(event=>{
   ...
   if(event.event==="duel_challenge_cancelled")setDuelNotice(tr(lang,"algoYolApp.raqibni_boshqa_oyinchi_oldi"));
 ...
 const enterSession=async(token:string,remember:boolean,isNew:boolean,refreshToken?:string)=>{
  ...
  if(!next){
   ...
   setAuthNotice(tr(lang,"algoYolApp.sessiya_tasdiqlanmadi_iltimos_qaytadan_kir"));
   setView("auth");
   return;
  }
 ...
 /* A profile links to the problems it lists by bank id; an id the bank does not
    hold (a problem since removed) is simply not a link there. */
 const openProblemById=(id:string)=>{const p=bankProblems.find(x=>x.id===id);if(p)openProblem(p,true)};
 ...
 const onAcceptChallenge=async(id:string)=>{
  ...
  setDuelNotice(reason==="expired"?(tr(lang,"algoYolApp.chaqiriq_muddati_tugadi"))
   :reason==="already_taken"?(tr(lang,"algoYolApp.kech_qoldingiz_raqibni_boshqa_oyinchi_oldi"))
   :(tr(lang,"algoYolApp.chaqiriqni_qabul_qilib_bolmadi")));
  await pullDuel();
 };
 ...
 useEffect(()=>{
  ...
  setAuthNotice(tr(lang,"algoYolApp.google_javobi_pkce_rejimida_qaytdi_adminis"));
  setView("auth");
 ...
 {signed&&duel?.status==="searching"&&!duelOnline&&<div className="duel-toast" role="status">{tr(lang,"algoYolApp.aloqa_uzildi_qayta_ulanmoqda")}</div>}
 </div>
}
  ...
  <section className="dash">
   <div className="dash-main">
    <div className="section-head">
     <h2>{tr(lang,"algoYolApp.sizga_tavsiya_etilgan_masalalar")}</h2>
   ...
   <aside className="dash-side">
    <div className="panel dash-path">
     <div className="dash-path-top">
      <h3>{tr(lang,"algoYolApp.sizning_yolingiz")}</h3>
     ...
     <a className="see-all" href="/roadmaps" onClick={linkTo(()=>go("roadmaps"))}>
      {tr(lang,"algoYolApp.barcha_yonalishlar")}
     </a>
    </div>
   </aside>
  </section>
...
function LandingLoop({lang}:{lang:Lang}){
 ...
 return <section className="lp-block">
  ...
  <ol className="loop">{L.how.map(([title,body],i)=>
   ...
   <li className="loop-back" aria-hidden>{tr(lang,"algoYolApp.va_yana_boshidan")}</li>
  </ol>
 </section>;
}
 ...
 return <>
  <section className="hero">
   <div className="hero-copy">
    ...
    <h1>{tr(lang,"algoYolApp.algoritmlarni_ona_tilingizda_organing_va_b")}</h1>
    <p>{L.what}</p>
...
function PlatformStats({lang}:{lang:Lang}){
 ...
 return <div className="hero-facts">
  <span><b>{size.tracks}</b> {tr(lang,"algoYolApp.yonalish")}</span>
  <span><b>{size.units}</b> {tr(lang,"algoYolApp.bosqich")}</span>
  <span><b>C++</b> {tr(lang,"algoYolApp.va_python")}</span>
  {learners!==null&&learners>=500&&
   <span><b>{String(learners).replace(/\B(?=(\d{3})+(?!\d))/g," ")}</b> {tr(lang,"algoYolApp.oquvchi")}</span>}
 </div>;
}
 ...
 return <div className="grid">{roads.map(r=>{
  ...
  return <a className={`road-card road-${status}`} key={r.slug} href={`/roadmaps/${r.slug}`}
   ...
   <span className="road-meta">
    <span>{done}/{total} {tr(lang,"algoYolApp.bosqich")}</span>
...
function Problems({lang,filter,setFilter,items,go,onSelect}:{lang:Lang,filter:string,setFilter:(x:string)=>void,items:BankProblem[],go:(v:View)=>void,onSelect:(p:BankProblem)=>void}){
 ...
 useEffect(()=>{
  const read=()=>{setMastery(loadMastery());setStatuses(loadProblemStatuses())};
  read();window.addEventListener("algoyol-progress",read);
  return()=>window.removeEventListener("algoyol-progress",read);
 },[]);
 ...
 /* Green means accepted, red means sent and not accepted yet, and nothing
    ...
    from before submissions were synced does not lose its tick. */
 const statusOf=(p:BankProblem):"solved"|"attempted"|"unsolved"=>
  mastery.evidence[`problem:${p.id}`]!==undefined?"solved":(statuses[p.judge||""]||"unsolved");
 const isSolved=(p:BankProblem)=>statusOf(p)==="solved";
 const shown=useMemo(()=>{
 ...
 return <>
  <div className="page-head">
   <div>
    <h1 className="page-title">{tr(lang,"algoYolApp.masalalar")}</h1>
    <p className="muted">{tr(lang,"algoYolApp.har_bir_yechim_mavzu_mahoratiga_otadi")}</p>
   </div>
  ...
  <div className="pb-layout">
   <aside className="pb-filters">
    <input className="rm-search" value={query} onChange={e=>setQuery(e.target.value)}
     placeholder={tr(lang,"algoYolApp.masala_qidirish")} aria-label={tr(lang,"algoYolApp.masala_qidirish_2")}/>
    ...
    {dirty&&<button className="ghost pb-clear" onClick={clear}>{tr(lang,"algoYolApp.filtrlarni_tozalash")}</button>}
   </aside>
   ...
   <div className="pb-results">
    {shown.length
     ? <table className="pb-table">
        <thead><tr>
         <th className="pb-th-status"><span className="sr-only">{tr(lang,"algoYolApp.holat")}</span></th>
         <th>{tr(lang,"algoYolApp.masala")}</th>
         <th className="pb-th-rating">{tr(lang,"algoYolApp.reyting_2")}</th>
         <th className="pb-th-topic">{tr(lang,"algoYolApp.mavzu")}</th>
        </tr></thead>
        <tbody>{shown.map(p=>{
         const status=statusOf(p),solved=status==="solved";
         const statusLabel=solved?tr(lang,"algoYolApp.yechilgan")
          :status==="attempted"?(uz?"Urinilgan — hali yechilmagan":"Attempted — not solved yet")
          :tr(lang,"algoYolApp.yechilmagan");
         return <tr key={p.id} className={status==="unsolved"?"":status} onClick={()=>onSelect(p)} tabIndex={0}
          onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();onSelect(p)}}}>
          <td><span className={`pb-status ${status}`} aria-label={statusLabel} title={statusLabel}>{solved?"✓":status==="attempted"?"✕":"○"}</span></td>
...
/* The problem and your submissions to it are two addresses —
   /problem/A02 and /problem/A02/submissions — the way Codeforces splits a
   problem from "My submissions". The history used to sit under the editor,
   where it grew a screen longer with every attempt and pushed itself out of
   sight; a tab under the title is where a solver looks for it, and a real href
   means a modified click opens it in a new browser tab. */
type ProblemSection="problem"|"problem-submissions";
function ProblemHead({lang,item,solved,section,onSection}:{lang:Lang;item:BankProblem;solved:boolean;section:ProblemSection;onSection:(v:ProblemSection)=>void}){
...
function ProblemSubmissionsPage({lang,item,signed,authLoading,verdict,onSection,onSignIn,onReuse}:{lang:Lang;item:BankProblem;signed:boolean;authLoading:boolean;verdict:string;onSection:(v:ProblemSection)=>void;onSignIn:()=>void;onReuse:(source:string,language:"cpp20"|"python3")=>void}){
 ...
 /* The history reloads on a verdict, not on the progress lines that lead up to
    one: "3/12 tests" is the same submission still being judged. A submission
    sent just before switching tabs keeps being judged in the app, and lands
    here when it settles. */
 const settledVerdict=/tekshirilmoqda|navbat|judging|queue/i.test(verdict)?"":verdict;
 return <><a className="crumb crumb-btn" href={`/problem/${item.id}`} onClick={e=>{if(e.metaKey||e.ctrlKey||e.shiftKey||e.button!==0)return;e.preventDefault();onSection("problem")}}>← {item.id}</a>
 <ProblemHead lang={lang} item={item} solved={solved} section="problem-submissions" onSection={onSection}/>
 {authLoading?<ScreenLoading lang={lang}/>
  :!signed?<div className="panel" style={{maxWidth:680}}><div className="notice">{lang==="uz"?"Yuborishlar tarixi hisobingizga bog'langan.":"Your submission history belongs to your account."} <button className="link-btn" onClick={onSignIn}>{lang==="uz"?"Kirish":"Sign in"}</button></div></div>
  :item.judge?<ProblemSubmissions lang={lang} problemKey={item.judge} signedIn={signed} reloadKey={settledVerdict} onReuse={onReuse}/>
  :null}</>;
}

function Problem({lang,item,code,setCode,codeLang,setCodeLang,verdict,submit,onBack,go,signed,onSection}:{lang:Lang;item:BankProblem;code:string;setCode:(x:string)=>void;codeLang:"cpp20"|"python3";setCodeLang:(x:"cpp20"|"python3")=>void;verdict:string;submit:()=>void;onBack:()=>void;go:(v:View)=>void;signed:boolean;onSection:(v:ProblemSection)=>void}){
...
function ScreenLoading({lang}:{lang:Lang}){
 return <div className="screen-state" role="status" aria-live="polite"><span className="spinner" aria-hidden/><p className="muted">{tr(lang,"algoYolApp.sessiya_tekshirilmoqda")}</p></div>;
}
...
function Leaderboard({lang,me,signed,onOpenPerson}:{lang:Lang;me:Profile|null;signed:boolean;onOpenPerson:(handle:string)=>void}){
 ...
 <div className="leader-tools">
  ...
  {signed&&!searchOn&&<div className="leader-modes">
   <button className={mode==="top"?"active":""} onClick={()=>setMode("top")}>{tr(lang,"algoYolApp.reyting")}</button>
   <button className={mode==="friends"?"active":""} onClick={()=>setMode("friends")}>{tr(lang,"algoYolApp.dostlarim")} ★</button>
  </div>}
 </div>
...
function Admin({lang,profile}:{lang:Lang,profile:Profile}){
  ...
  if(!can(role,"content.view_management"))return <><div className="page-head"><div><span className="tag">ADMIN STUDIO</span><h1 className="page-title" style={{marginTop:12}}>{tr(lang,"algoYolApp.ruxsat_yoq")}</h1></div></div><div className="panel"><div className="notice">{tr(lang,"algoYolApp.bu_sahifa_faqat_admin_va_owner_rollari_uch")}<b>{roleLabel(role,lang)}</b>.</div></div></>;
 return <><div className="page-head"><div><span className="tag">ADMIN STUDIO · {roleLabel(role,lang)}</span><h1 className="page-title" style={{marginTop:12}}>{tr(lang,"algoYolApp.yangi_masala")}</h1></div><button className="primary" disabled title={tr(lang,"algoYolApp.masala_muharriri_hali_ulanmagan")}>{tr(lang,"algoYolApp.qoralamani_saqlash")}</button></div><div className="panel"><div className="admin-grid"><div className="field"><label>O‘zbekcha nomi</label><input placeholder="Masala nomi"/></div><div className="field"><label>English title</label><input placeholder="Problem title"/></div><div className="field"><label>{tr(lang,"algoYolApp.qiyinlik")}</label><select><option>Easy · 100</option><option>Medium · 200</option><option>Hard · 300</option></select></div><div className="field"><label>Taglar</label><input placeholder="binary-search, arrays"/></div></div><div className="admin-grid"><div className="field"><label>O‘zbekcha shart</label><textarea rows={9} placeholder="Masala shartini yozing…"/></div><div className="field"><label>English statement</label><textarea rows={9} placeholder="Write the problem statement…"/></div></div><div className="admin-grid"><div className="field"><label>{tr(lang,"algoYolApp.vaqt_chegarasi")}</label><input value="1000 ms" readOnly/></div><div className="field"><label>{tr(lang,"algoYolApp.xotira_chegarasi")}</label><input value="256 MB" readOnly/></div></div><div className="field"><label>{tr(lang,"algoYolApp.yashirin_testlar")}</label><textarea rows={5} placeholder="Input → Expected output"/></div><div className="notice notice-info">{tr(lang,"algoYolApp.bu_forma_korib_chiqish_rejimida_masala_muh")}</div></div></>}
