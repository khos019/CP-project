...
cout<<ans<<"\\n";`),
    // Counting the edges answers how many pairs are directly joined rather than how many reach each other; summing the sizes without pairing them counts vertices, not pairs.
    wrong: [cpp(`int n,m;cin>>n>>m;long long c=0;
for(int i=0;i<m;++i){int a,b;cin>>a>>b;++c;}
cout<<c<<"\\n";`), cpp(`int n,m;cin>>n>>m;vector<int>p(n+1);iota(p.begin(),p.end(),0);
vector<long long>sz(n+1,1);
function<int(int)>f=[&](int x){while(p[x]!=x){p[x]=p[p[x]];x=p[x];}return x;};
for(int i=0;i<m;++i){int a,b;cin>>a>>b;int ra=f(a),rb=f(b);
 if(ra!=rb){p[ra]=rb;sz[rb]+=sz[ra];}}
...
cout<<ans<<"\\n";`)],
  ...
  "dp-knapsack-count-ways": {
    solution: cpp(`long long n,W;cin>>n>>W;const long long M=1000000007;
vector<long long>dp(W+1,0);dp[0]=1;
for(long long i=0;i<n;++i){long long w;cin>>w;
 for(long long c=W;c>=w;--c)dp[c]=(dp[c]+dp[c-w])%M;}
cout<<dp[W]<<"\\n";`),
    // Walking the capacity upwards lets one item be taken many times, which counts multisets instead of sets; starting the table at zero forgets that the empty set is one way to weigh nothing.
    wrong: [cpp(`long long n,W;cin>>n>>W;const long long M=1000000007;
vector<long long>dp(W+1,0);dp[0]=1;
for(long long i=0;i<n;++i){long long w;cin>>w;
 for(long long c=w;c<=W;++c)dp[c]=(dp[c]+dp[c-w])%M;}
cout<<dp[W]<<"\\n";`), cpp(`long long n,W;cin>>n>>W;const long long M=1000000007;
...
for(int j=0;j<n;++j)for(int i=0;i<j;++i){
 ...
 best=max(best,len);}
cout<<best<<"\\n";`),
    // Requiring the elements to be adjacent measures a run rather than a subsequence; never reading the chain that already ends at i means every progression restarts at length two.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
if(n<3){cout<<n<<"\\n";return 0;}
int best=2,cur=2;
for(int i=2;i<n;++i){
...
cout<<best<<"\\n";`)],
  ...
  "math-count-lattice-in-circle": {
    solution: cpp(`long long r;cin>>r;long long total=0;
for(long long x=-r;x<=r;++x){
 ...
 total+=2*y+1;}
cout<<total<<"\\n";`),
    // Counting only the points strictly inside drops the whole boundary, which the statement includes; counting one side of the y axis halves the answer.
    wrong: [cpp(`long long r;cin>>r;long long total=0;
for(long long x=-r;x<=r;++x){
 ...
 while((y+1)*(y+1)<=rem)++y;
 total+=2*y+1;}
cout<<total<<"\\n";`), cpp(`long long r;cin>>r;long long total=0;
...
for(int v=1;v<=n;++v){long long big=n-sz[v];
 ...
 if(big<=n/2)++c;}
cout<<c<<"\\n";`),
    // Forgetting the component above the node judges only what hangs below it; a strict comparison rejects the node that lands exactly on half.
    wrong: [cpp(`int n;cin>>n;vector<vector<int>>g(n+1);
for(int i=0;i<n-1;++i){int a,b;cin>>a>>b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n+1,0),order;vector<char>seen(n+1,0);vector<long long>sz(n+1,1);
vector<int>st{1};seen[1]=1;
...
cout<<c<<"\\n";`)],
  ...
  "dp-lcs-three-strings": {
    solution: cpp(`string a,b,c;cin>>a>>b>>c;
...
cout<<dp[n][m][k]<<"\\n";`),
    // Taking the longest common subsequence of the first two and then matching it against the third is not the same problem; comparing only two of the three strings ignores one of them entirely.
    wrong: [cpp(`string a,b,c;cin>>a>>b>>c;
...
cout<<llabs(area)<<"\\n";`),
    // Running the shoelace formula over the points in their input order measures a self-crossing polygon rather than the hull; halving the doubled area throws away the half that made it an integer.
    wrong: [cpp(`int n;cin>>n;vector<pair<long long,long long>>p(n);
for(auto&q:p)cin>>q.first>>q.second;
long long area=0;
for(int i=0;i<n;++i){int j=(i+1)%n;
 area+=p[i].first*p[j].second-p[j].first*p[i].second;}
cout<<llabs(area)<<"\\n";`), cpp(`int n;cin>>n;vector<pair<long long,long long>>p(n);
for(auto&q:p)cin>>q.first>>q.second;
sort(p.begin(),p.end());p.erase(unique(p.begin(),p.end()),p.end());
int m=p.size();
if(m<3){cout<<0<<"\\n";return 0;}
...
cout<<(llabs(area)/2)<<"\\n";`)],
  ...
  "bt-count-hamiltonian-paths": {
    solution: cpp(`int n,m;cin>>n>>m;vector<int>adj(n,0);
...
cout<<total<<"\\n";`),
    // Halving the total assumes every path has a distinct reverse, which is false for a single vertex; fixing the start at vertex 1 counts only the paths that begin there.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<int>adj(n,0);
for(int i=0;i<m;++i){int a,b;cin>>a>>b;--a;--b;adj[a]|=1<<b;adj[b]|=1<<a;}
vector<vector<long long>>dp(1<<n,vector<long long>(n,0));
for(int v=0;v<n;++v)dp[1<<v][v]=1;
for(int mask=1;mask<(1<<n);++mask)for(int v=0;v<n;++v){
...
long long total=0;
for(int v=0;v<n;++v)total+=dp[(1<<n)-1][v];
cout<<(total/2)<<"\\n";`), cpp(`int n,m;cin>>n>>m;vector<int>adj(n,0);
for(int i=0;i<m;++i){int a,b;cin>>a>>b;--a;--b;adj[a]|=1<<b;adj[b]|=1<<a;}
vector<vector<long long>>dp(1<<n,vector<long long>(n,0));
dp[1][0]=1;
for(int mask=1;mask<(1<<n);++mask)for(int v=0;v<n;++v){
...
cout<<(min(i,j)+1)<<"\\n";`),
    // Comparing only the first character of each rotation stops at the first tie; building every rotation as a string is correct but quadratic, and here it is wrong in a different way — it keeps the last of the equal ones rather than the first.
    wrong: [cpp(`string s;cin>>s;int n=s.size();
int best=0;
...
cout<<(best+1)<<"\\n";`)],
  ...
  "sqrt-decomp-range-sum": {
    solution: cpp(`long long n,q;cin>>n>>q;vector<long long>a(n);for(auto&x:a)cin>>x;
...
while(q--){long long t;cin>>t;
 if(t==1){long long i,x;cin>>i>>x;--i;bs[i/B]+=x-a[i];a[i]=x;}
  ...
  while(l<=r){s+=a[l];++l;}
  cout<<s<<"\\n";}}`),
    // Adding the new value to the block instead of the difference lets the block sum drift away from the array; walking whole blocks without checking that the block ends inside the range overshoots the right edge.
    wrong: [cpp(`long long n,q;cin>>n>>q;vector<long long>a(n);for(auto&x:a)cin>>x;
long long B=max(1LL,(long long)sqrtl((long double)n));
long long nb=(n+B-1)/B;vector<long long>bs(nb,0);
for(long long i=0;i<n;++i)bs[i/B]+=a[i];
while(q--){long long t;cin>>t;
 if(t==1){long long i,x;cin>>i>>x;--i;bs[i/B]+=x;a[i]=x;}
  ...
  while(l+B-1<=r){s+=bs[l/B];l+=B;}
  while(l<=r){s+=a[l];++l;}
  cout<<s<<"\\n";}}`), cpp(`long long n,q;cin>>n>>q;vector<long long>a(n);for(auto&x:a)cin>>x;
long long B=max(1LL,(long long)sqrtl((long double)n));
long long nb=(n+B-1)/B;vector<long long>bs(nb,0);
for(long long i=0;i<n;++i)bs[i/B]+=a[i];
while(q--){long long t;cin>>t;
...
cout<<total<<"\\n";`),
    // Dropping the absolute value lets the descents cancel the climbs and collapses to the difference between the ends; the distance from the first element to the last ignores the route.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long total=0;
...
cout<<llabs(a[n-1]-a[0])<<"\\n";`)],
  ...
  "is-perfect-square": {
    solution: cpp(`long long n;cin>>n;
long long k=(long long)sqrtl((long double)n);
while(k>0&&k*k>n)--k;
while((k+1)*(k+1)<=n)++k;
cout<<((k*k==n)?"YES":"NO")<<"\\n";`),
    // The last digit of a square is never 2, 3, 7 or 8, but ruling those out is only half a test — it says yes to everything else; treating 0 as a special case that is not a square contradicts the statement.
    wrong: [cpp(`long long n;cin>>n;long long d=n%10;
bool maybe=(d!=2&&d!=3&&d!=7&&d!=8);
cout<<(maybe?"YES":"NO")<<"\\n";`), cpp(`long long n;cin>>n;
if(n==0){cout<<"NO\\n";return 0;}
long long k=(long long)sqrtl((long double)n);
while(k>0&&k*k>n)--k;
while((k+1)*(k+1)<=n)++k;
cout<<((k*k==n)?"YES":"NO")<<"\\n";`)],
  ...
  "str-count-adjacent-equal": {
    solution: cpp(`string s;cin>>s;long long c=0;
for(size_t i=1;i<s.size();++i)if(s[i]==s[i-1])++c;
cout<<c<<"\\n";`),
    // Counting the runs of equal letters rather than the pairs inside them reports one for a run of three; skipping past a match avoids the overlap the statement allows.
    wrong: [cpp(`string s;cin>>s;long long c=0;
...
stable_sort(a.begin(),a.end(),[&](long long x,long long y){
 ...
 return x<y;});
for(int i=0;i<n;++i)cout<<a[i]<<(i+1<n?" ":"\\n");`),
    // Leaving the tie unbroken keeps whatever order the input happened to have; sorting by value alone ignores the digit sums entirely.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
auto ds=[](long long v){long long s=0;while(v){s+=v%10;v/=10;}return s;};
...
while(i<n&&j<m){
 if(a[i]<b[j])++i;
  ...
  while(j<m&&b[j]==v)++j;}}
cout<<c<<"\\n";`),
    // Advancing one step at a time on a match counts a shared value once per repeated copy; comparing the arrays position by position only finds values that happen to line up.
...
cout<<c<<"\\n";`)],
  ...
  "bt-ways-climb-k-steps": {
    solution: cpp(`long long n,k;cin>>n>>k;const long long M=1000000007;
vector<long long>dp(n+1,0);dp[0]=1;
for(long long i=1;i<=n;++i)
 for(long long s=1;s<=k&&s<=i;++s)dp[i]=(dp[i]+dp[i-s])%M;
cout<<dp[n]<<"\\n";`),
    // Starting the table at zero instead of one leaves every count at zero; allowing a move of size zero adds an infinite loop's worth of ways, here showing up as the count for k+1 sizes.
    wrong: [cpp(`long long n,k;cin>>n>>k;const long long M=1000000007;
...
cout<<dp[n]<<"\\n";`)],
  ...
  "geo-two-rect-union-area": {
    solution: cpp(`long long ax1,ay1,ax2,ay2,bx1,by1,bx2,by2;
...
cout<<(area-ox*oy)<<"\\n";`),
    // Adding the two areas counts the overlap twice; letting a negative overlap through subtracts an area that does not exist when the rectangles are apart.
    wrong: [cpp(`long long ax1,ay1,ax2,ay2,bx1,by1,bx2,by2;
cin>>ax1>>ay1>>ax2>>ay2>>bx1>>by1>>bx2>>by2;
cout<<((ax2-ax1)*(ay2-ay1)+(bx2-bx1)*(by2-by1))<<"\\n";`), cpp(`long long ax1,ay1,ax2,ay2,bx1,by1,bx2,by2;
cin>>ax1>>ay1>>ax2>>ay2>>bx1>>by1>>bx2>>by2;
long long area=(ax2-ax1)*(ay2-ay1)+(bx2-bx1)*(by2-by1);
...
cout<<groups<<"\\n";`),
    // Comparing against the previous element rather than the start of the group lets a group drift arbitrarily wide; a strict comparison starts a new group at a spread that was still allowed.
    wrong: [cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
long long groups=0;
for(long long i=0;i<n;++i){
...
cout<<groups<<"\\n";`)],
  ...
  "bs-min-speed-finish": {
    solution: cpp(`long long n,H;cin>>n>>H;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<lo<<"\\n";`),
    // Dividing without rounding up pretends a part-used hour is free; searching from zero admits a speed of zero, which finishes nothing.
    wrong: [cpp(`long long n,H;cin>>n>>H;vector<long long>a(n);for(auto&x:a)cin>>x;
auto hours=[&](long long v){long long h=0;for(long long x:a)h+=x/v;return h;};
long long lo=1,hi=*max_element(a.begin(),a.end());
while(lo<hi){long long mid=lo+(hi-lo)/2;
 if(hours(mid)<=H)hi=mid;else lo=mid+1;}
cout<<lo<<"\\n";`), cpp(`long long n,H;cin>>n>>H;vector<long long>a(n);for(auto&x:a)cin>>x;
auto hours=[&](long long v){long long h=0;for(long long x:a)h+=(x+v-1)/v;return h;};
...
cout<<lo<<"\\n";`)],
  ...
  "tree-max-path-sum-any": {
    solution: cpp(`int n;cin>>n;vector<long long>val(n+1);
for(int i=1;i<=n;++i)cin>>val[i];
vector<vector<int>>g(n+1);
for(int i=0;i<n-1;++i){int a,b;cin>>a>>b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n+1,0),order;vector<char>seen(n+1,0);
...
cout<<best<<"\\n";`),
    // Taking only the best single branch measures a path that ends at the node instead of passing through it; clamping the node's own value at zero loses the all-negative case, where the answer is a single node.
    wrong: [cpp(`int n;cin>>n;vector<long long>val(n+1);
for(int i=1;i<=n;++i)cin>>val[i];
vector<vector<int>>g(n+1);
for(int i=0;i<n-1;++i){int a,b;cin>>a>>b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n+1,0),order;vector<char>seen(n+1,0);
...
cout<<best<<"\\n";`), cpp(`int n;cin>>n;vector<long long>val(n+1);
for(int i=1;i<=n;++i)cin>>val[i];
vector<vector<int>>g(n+1);
for(int i=0;i<n-1;++i){int a,b;cin>>a>>b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n+1,0),order;vector<char>seen(n+1,0);
...
cout<<ans<<"\\n";`)],
  ...
  "num-digit-spread": {
    solution: cpp(`string s;cin>>s;int mx=0,mn=9;
for(char c:s){int d=c-'0';if(d>mx)mx=d;if(d<mn)mn=d;}
cout<<(mx-mn)<<"\\n";`),
    // Starting the smallest digit at zero pins it there whatever the number holds, so the answer becomes the largest digit alone; the first and the last digit say nothing about the digits between them.
    wrong: [cpp(`string s;cin>>s;int mx=0,mn=0;
for(char c:s){int d=c-'0';if(d>mx)mx=d;}
cout<<(mx-mn)<<"\\n";`), cpp(`string s;cin>>s;
int a=s.front()-'0',b=s.back()-'0';
cout<<((a>b)?(a-b):(b-a))<<"\\n";`)],
  },
  "grid-count-border-cells": {
    solution: cpp(`int n,m;cin>>n>>m;long long c=0;
for(int i=0;i<n;++i)for(int j=0;j<m;++j){int v;cin>>v;
 if(v&&(i==0||i==n-1||j==0||j==m-1))++c;}
cout<<c<<"\\n";`),
    // Adding the four sides up separately counts each corner twice, and counts a single row twice over; the whole table minus its interior is right only when the interior is a real rectangle.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<vector<int>>a(n,vector<int>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
long long c=0;
for(int j=0;j<m;++j){c+=a[0][j];c+=a[n-1][j];}
for(int i=0;i<n;++i){c+=a[i][0];c+=a[i][m-1];}
cout<<c<<"\\n";`), cpp(`int n,m;cin>>n>>m;vector<vector<int>>a(n,vector<int>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
...
cout<<(all-inner-1)<<"\\n";`)],
  ...
  "str-count-same-end-substrings": {
    solution: cpp(`string s;cin>>s;vector<long long>cnt(26,0);
for(char c:s)++cnt[c-'a'];
long long t=0;
for(int i=0;i<26;++i)t+=cnt[i]*(cnt[i]+1)/2;
cout<<t<<"\\n";`),
    // Counting the single letters plus the equal neighbours misses every pair of matching letters standing further apart; the pair count alone leaves out the one-letter substrings the statement asks to include.
    wrong: [cpp(`string s;cin>>s;long long t=(long long)s.size();
for(size_t i=0;i+1<s.size();++i)if(s[i]==s[i+1])++t;
cout<<t<<"\\n";`), cpp(`string s;cin>>s;vector<long long>cnt(26,0);
for(char c:s)++cnt[c-'a'];
long long t=0;
for(int i=0;i<26;++i)t+=cnt[i]*(cnt[i]-1)/2;
cout<<t<<"\\n";`)],
  },
  "greedy-min-increments-strict-increase": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<steps<<"\\n";`),
    // Allowing an element to merely match the one before it leaves equal neighbours the statement forbids; measuring each shortfall against the original neighbour rather than its raised value understates every step after the first.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long prev=a[0],steps=0;
for(int i=1;i<n;++i){
...
cout<<steps<<"\\n";`)],
  ...
  "prefix-longest-balanced-zero-one": {
    solution: cpp(`string s;cin>>s;int n=(int)s.size();
...
cout<<best<<"\\n";`),
    // Remembering the latest place a prefix sum appeared measures the distance to the nearest repeat rather than the farthest; leaving out the empty prefix loses every balanced stretch that starts at the very beginning.
    wrong: [cpp(`string s;cin>>s;int n=(int)s.size();
...
for(int i=0;i<n;++i){
 ...
 last[bal]=i;}
cout<<best<<"\\n";`), cpp(`string s;cin>>s;int n=(int)s.size();
...
cout<<best<<"\\n";`),
    // Pairing every opening bracket with every closing one ignores the order they arrive in, so it counts brackets that could never match; a single left-to-right balance never recovers the tail of a run that an unmatched opening bracket left hanging.
    wrong: [cpp(`string s;cin>>s;long long o=0,c=0;
for(char ch:s){if(ch=='(')++o;else ++c;}
cout<<(2*min(o,c))<<"\\n";`), cpp(`string s;cin>>s;long long o=0,c=0,best=0;
for(char ch:s){
 if(ch=='(')++o;else ++c;
...
cout<<best<<"\\n";`)],
  ...
  "greedy-min-deletions-unique-freq": {
    solution: cpp(`string s;cin>>s;vector<long long>cnt(26,0);
for(char c:s)++cnt[c-'a'];
vector<long long>f;
...
for(long long v:f){
 ...
 allow=cur-1;}
cout<<del<<"\\n";`),
    // Letting the allowance run below zero charges for deletions that were already made once the letter is gone; counting how many frequencies repeat says how many letters must change, not how many characters that costs.
    wrong: [cpp(`string s;cin>>s;vector<long long>cnt(26,0);
for(char c:s)++cnt[c-'a'];
vector<long long>f;
for(int i=0;i<26;++i)if(cnt[i]>0)f.push_back(cnt[i]);
sort(f.rbegin(),f.rend());
...
cout<<del<<"\\n";`)],
  ...
  "dp-max-sum-no-two-adjacent-circle": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
if(n==1){cout<<a[0]<<"\\n";return 0;}
...
cout<<max(line(0,n-2),line(1,n-1))<<"\\n";`),
    // Solving the row and ignoring the wrap lets the first and the last number be taken together, which the ring forbids; comparing the even positions against the odd ones misses every choice that leaves a gap of two.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<max(ev,od)<<"\\n";`)],
  ...
  "tree-remove-edges-even-components": {
    solution: cpp(`int n;cin>>n;vector<vector<int>>g(n);
...
cout<<c<<"\\n";`),
    // Counting the root's own subtree alongside the rest adds a cut that has no edge above it to make; counting the odd subtrees instead answers how many edges must be kept, which is the complement of what was asked.
    wrong: [cpp(`int n;cin>>n;vector<vector<int>>g(n);
for(int i=0;i+1<n;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n,-1),order;order.reserve(n);
vector<int>st{0};vector<char>seen(n,0);seen[0]=1;
while(!st.empty()){int v=st.back();st.pop_back();order.push_back(v);
...
cout<<c<<"\\n";`), cpp(`int n;cin>>n;vector<vector<int>>g(n);
for(int i=0;i+1<n;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n,-1),order;order.reserve(n);
vector<int>st{0};vector<char>seen(n,0);seen[0]=1;
while(!st.empty()){int v=st.back();st.pop_back();order.push_back(v);
...
for(int v=0;v<n;++v)cout<<max(down[v],up[v])<<(v+1<n?' ':'\\n');`),
    // Passing the parent's own downward depth to a child feeds the child's branch straight back to it, so a vertex is told the longest arm runs through itself; the diameter is the largest of these answers but not the answer for every vertex.
    wrong: [cpp(`int n;cin>>n;vector<vector<int>>g(n);
for(int i=0;i+1<n;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);g[b].push_back(a);}
vector<int>par(n,-1),order;order.reserve(n);
vector<int>st{0};vector<char>seen(n,0);seen[0]=1;
...
for(int v=0;v<n;++v)cout<<diam<<(v+1<n?' ':'\\n');`)],
  ...
  "time-minutes-between": {
    solution: cpp(`long long h1,m1,h2,m2;cin>>h1>>m1>>h2>>m2;
long long a=h1*60+m1,b=h2*60+m2;
long long d=b-a;
if(d<0)d+=24*60;
cout<<d<<"\\n";`),
    // Taking the absolute difference measures the shorter way round the clock rather than the time that actually passes; wrapping whenever the second time is not larger turns equal times into a whole day.
    wrong: [cpp(`long long h1,m1,h2,m2;cin>>h1>>m1>>h2>>m2;
long long a=h1*60+m1,b=h2*60+m2;
cout<<llabs(b-a)<<"\\n";`), cpp(`long long h1,m1,h2,m2;cin>>h1>>m1>>h2>>m2;
long long a=h1*60+m1,b=h2*60+m2;
long long d=b-a;
if(d<=0)d+=24*60;
cout<<d<<"\\n";`)],
...
cout<<c<<"\\n";`),
    // Comparing a peak against its neighbours without insisting on a strict rise lets a plateau count as one; counting the ends as peaks whenever they beat their single neighbour contradicts the rule the statement sets.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long c=0;
for(int i=1;i+1<n;++i)if(a[i]>=a[i-1]&&a[i]>=a[i+1])++c;
cout<<c<<"\\n";`), cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<c<<"\\n";`)],
  ...
  "math-count-carries": {
    solution: cpp(`long long a,b;cin>>a>>b;long long c=0,carry=0;
while(a>0||b>0){
 long long d=a%10+b%10+carry;
 if(d>=10){++c;carry=1;}else carry=0;
 a/=10;b/=10;}
cout<<c<<"\\n";`),
    // Adding the two digits without the carry arriving from the position before misses every carry that only a chain produces; comparing the length of the sum against the longer operand notices at most the one carry that spills off the end.
    wrong: [cpp(`long long a,b;cin>>a>>b;long long c=0;
while(a>0||b>0){
 if(a%10+b%10>=10)++c;
 a/=10;b/=10;}
cout<<c<<"\\n";`), cpp(`long long a,b;cin>>a>>b;
auto len=[](long long v){long long n=0;do{++n;v/=10;}while(v>0);return n;};
cout<<(len(a+b)-max(len(a),len(b)))<<"\\n";`)],
  },
  "str-min-chars-to-anagram": {
    solution: cpp(`string a,b;cin>>a>>b;vector<long long>ca(26,0),cb(26,0);
for(char c:a)++ca[c-'a'];
for(char c:b)++cb[c-'a'];
long long t=0;
for(int i=0;i<26;++i)t+=llabs(ca[i]-cb[i]);
cout<<t<<"\\n";`),
    // The difference of the two lengths only accounts for the surplus on one side, not for letters each string has that the other lacks; counting a letter once when the two counts disagree ignores how far apart they are.
    wrong: [cpp(`string a,b;cin>>a>>b;
cout<<llabs((long long)a.size()-(long long)b.size())<<"\\n";`), cpp(`string a,b;cin>>a>>b;vector<long long>ca(26,0),cb(26,0);
for(char c:a)++ca[c-'a'];
for(char c:b)++cb[c-'a'];
long long t=0;
...
cout<<t<<"\\n";`)],
  ...
  "sort-min-pair-diff-sum": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
long long s=0;
for(int i=0;i+1<n;i+=2)s+=a[i+1]-a[i];
cout<<s<<"\\n";`),
    // Pairing the smallest with the largest spreads every pair as wide as it can go, which maximises the total instead of minimising it; pairing the numbers in the order they arrive ignores that sorting is what brings the partners close together.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
long long s=0;
...
cout<<s<<"\\n";`)],
  ...
  "bs-kth-missing-positive": {
    solution: cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
long long lo=0,hi=n;
while(lo<hi){long long mid=lo+(hi-lo)/2;
 if(a[mid]-(mid+1)<k)lo=mid+1;else hi=mid;}
cout<<(lo+k)<<"\\n";`),
    // Adding k to the length assumes the array occupies the first n places, which stops being true as soon as a number is missing inside it; counting the values that come before the answer and then subtracting one lands on the number just below it.
    wrong: [cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
cout<<(n+k)<<"\\n";`), cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
long long lo=0,hi=n;
while(lo<hi){long long mid=lo+(hi-lo)/2;
...
cout<<(lo+k-1)<<"\\n";`)],
  ...
  "grid-count-enclosed-zero-regions": {
    solution: cpp(`int n,m;cin>>n>>m;vector<vector<int>>a(n,vector<int>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
...
for(int i=0;i<n;++i)for(int j=0;j<m;++j){
 ...
 if(!edge)++c;}
cout<<c<<"\\n";`),
    // Counting every region of zeros ignores the condition that it must be fenced in; joining cells diagonally as well merges regions the statement keeps apart.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<vector<int>>a(n,vector<int>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
vector<vector<char>>seen(n,vector<char>(m,0));
int dx[4]={1,-1,0,0},dy[4]={0,0,1,-1};
...
cout<<c<<"\\n";`)],
  ...
  "graph-shortest-path-one-free-edge": {
    solution: cpp(`int n,m;cin>>n>>m;
...
cout<<((best>=INF)?-1:best)<<"\\n";`),
    // Running an ordinary shortest path and then subtracting the heaviest edge on the graph discounts an edge the route may never use; dropping the most expensive edge of the plain cheapest route ignores that a different, pricier route can gain far more from the discount.
    wrong: [cpp(`int n,m;cin>>n>>m;
vector<vector<pair<int,long long>>>g(n);
...
cout<<ans<<"\\n";`),
    // Subtracting the multiples of each prime square one prime at a time removes a number divisible by two different squares twice over; ruling out only the multiples of four leaves every number divisible by nine or twenty-five still standing.
    wrong: [cpp(`long long n;cin>>n;
long long L=1;while((L+1)*(L+1)<=n)++L;
...
cout<<out;`),
    // Treating the pending value as an amount to add instead of a value to assign leaves the old contents underneath every later range; pushing the pending value down without clearing it makes the same assignment land again on a range that was overwritten since.
    wrong: [cpp(`int n,q;cin>>n>>q;
vector<long long>sum(4*n,0),lz(4*n,0);
function<void(int,int,int)>apply2=[&](int node,int l,int r){
 if(lz[node]==0)return;
...
cout<<out;`)],
  ...
  "array-min-max-product": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long mn=a[0],mx=a[0];
for(long long x:a){if(x<mn)mn=x;if(x>mx)mx=x;}
cout<<(mn*mx)<<"\\n";`),
    // The first and the last element are not the smallest and the largest unless the array happens to be sorted; picking the extremes by size regardless of sign confuses the most negative number with the largest one.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
cout<<(a[0]*a[n-1])<<"\\n";`), cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long mn=a[0],mx=a[0];
...
cout<<(mn*mx)<<"\\n";`)],
  ...
  "math-sum-divisors": {
    solution: cpp(`long long n;cin>>n;long long s=0;
for(long long d=1;d*d<=n;++d){
 if(n%d)continue;
 s+=d;
 if(d!=n/d)s+=n/d;}
cout<<s<<"\\n";`),
    // Adding both a divisor and its partner without checking whether they coincide counts the middle divisor of a perfect square twice; adding only the divisors below the square root leaves out every large one.
    wrong: [cpp(`long long n;cin>>n;long long s=0;
for(long long d=1;d*d<=n;++d){
 if(n%d)continue;
 s+=d;s+=n/d;}
cout<<s<<"\\n";`), cpp(`long long n;cin>>n;long long s=0;
for(long long d=1;d*d<=n;++d)if(n%d==0)s+=d;
cout<<s<<"\\n";`)],
  },
  "math-base-digit-sum": {
    solution: cpp(`long long n,b;cin>>n>>b;long long s=0;
while(n>0){s+=n%b;n/=b;}
cout<<s<<"\\n";`),
    // Peeling digits with tens rather than the given base answers the question for base ten whatever b says; stopping while the number is merely above the base drops its leading digit.
    wrong: [cpp(`long long n,b;cin>>n>>b;long long s=0;
while(n>0){s+=n%10;n/=10;}
cout<<s<<"\\n";`), cpp(`long long n,b;cin>>n>>b;long long s=0;
while(n>b){s+=n%b;n/=b;}
cout<<s<<"\\n";`)],
  },
  "two-pointers-pairs-sum-range": {
    solution: cpp(`long long n,L,R;cin>>n>>L>>R;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
...
cout<<(atMost(R)-atMost(L-1))<<"\\n";`),
    // Counting each pair from both ends doubles every answer, since the statement treats a pair as unordered; requiring the sum to be strictly inside the bounds drops the pairs that land exactly on them.
    wrong: [cpp(`long long n,L,R;cin>>n>>L>>R;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
auto atMost=[&](long long lim){long long c=0;int i=0,j=(int)n-1;
 while(i<j){
  if(a[i]+a[j]<=lim){c+=j-i;++i;}
 ...
 return c;};
cout<<(2*(atMost(R)-atMost(L-1)))<<"\\n";`), cpp(`long long n,L,R;cin>>n>>L>>R;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
auto atMost=[&](long long lim){long long c=0;int i=0,j=(int)n-1;
 while(i<j){
  if(a[i]+a[j]<=lim){c+=j-i;++i;}
...
cout<<(atMost(R-1)-atMost(L))<<"\\n";`)],
  ...
  "greedy-min-swaps-balance-brackets": {
    solution: cpp(`string s;cin>>s;long long bal=0,worst=0;
for(char c:s){
 bal+=(c=='[')?1:-1;
 if(-bal>worst)worst=-bal;}
cout<<((worst+1)/2)<<"\\n";`),
    // The deepest shortfall is how many closings stand unmatched, but one swap repairs two of them at once, so reporting it directly doubles the answer; counting the positions where the balance is negative measures how long the shortfall lasts rather than how deep it goes.
    wrong: [cpp(`string s;cin>>s;long long bal=0,worst=0;
for(char c:s){
 bal+=(c=='[')?1:-1;
 if(-bal>worst)worst=-bal;}
cout<<worst<<"\\n";`), cpp(`string s;cin>>s;long long bal=0,c=0;
for(char ch:s){
 bal+=(ch=='[')?1:-1;
 if(bal<0)++c;}
cout<<c<<"\\n";`)],
  ...
  "dp-count-dice-sums": {
    solution: cpp(`long long n,f,s;cin>>n>>f>>s;const long long M=1000000007;
...
for(long long i=0;i<n;++i){
 ...
 dp=nd;}
cout<<dp[s]<<"\\n";`),
    // Letting a die show zero adds outcomes no die can produce; treating the top face as out of reach shrinks every die to f − 1 faces.
    wrong: [cpp(`long long n,f,s;cin>>n>>f>>s;const long long M=1000000007;
vector<long long>dp(s+1,0);dp[0]=1;
for(long long i=0;i<n;++i){
 vector<long long>pre(s+2,0);
 for(long long v=0;v<=s;++v)pre[v+1]=(pre[v]+dp[v])%M;
 ...
 for(long long v=0;v<=s;++v){
  ...
  nd[v]=((pre[hi+1]-pre[lo])%M+M)%M;}
 dp=nd;}
cout<<dp[s]<<"\\n";`), cpp(`long long n,f,s;cin>>n>>f>>s;const long long M=1000000007;
vector<long long>dp(s+1,0);dp[0]=1;
for(long long i=0;i<n;++i){
 vector<long long>pre(s+2,0);
 for(long long v=0;v<=s;++v)pre[v+1]=(pre[v]+dp[v])%M;
...
for(int i=1;i<m;++i){int j=f[i-1];
 ...
 f[i]=j;}
cout<<(n-f[m-1])<<"\\n";`),
    // Matching the string against its own reverse the other way round finds the longest palindromic prefix, which is what you would need in order to add letters at the front; falling back on the whole length whenever the string is not already a palindrome ignores every palindromic tail it does have.
    wrong: [cpp(`string s;cin>>s;int n=(int)s.size();
string r=s;reverse(r.begin(),r.end());
string t=s+"#"+r;
int m=(int)t.size();
...
cout<<ans<<"\\n";`),
    // Looking only at the sums that pair one array's first element with the other's misses every sum built from two middling entries; stopping the heap one pop early reports the sum before the one asked for.
    wrong: [cpp(`long long n,m,k;cin>>n>>m>>k;
vector<long long>a(n),b(m);
for(auto&x:a)cin>>x;for(auto&x:b)cin>>x;
vector<long long>v;
...
cout<<ans<<"\\n";`)],
  ...
  "graph-functional-longest-cycle": {
    solution: cpp(`int n;cin>>n;vector<int>f(n);
...
cout<<best<<"\\n";`),
    // Reporting how many cycles the graph holds answers a different question than how long the longest one is; walking forward from each vertex until a repeat counts the tail leading into the cycle along with the cycle itself.
    wrong: [cpp(`int n;cin>>n;vector<int>f(n);
for(int i=0;i<n;++i){cin>>f[i];--f[i];}
vector<int>state(n,0),ord(n,0);
long long cycles=0;
for(int s=0;s<n;++s){
 ...
 if(state[v]==1)++cycles;
 for(int u:path)state[u]=2;}
cout<<cycles<<"\\n";`), cpp(`int n;cin>>n;vector<int>f(n);
for(int i=0;i<n;++i){cin>>f[i];--f[i];}
long long best=0;
for(int s=0;s<n;++s){
...
cout<<lo<<"\\n";`),
    // Counting the stretches that stay strictly below a candidate shifts the search one value past the answer; looking only at the stretches that begin at the very first element leaves out almost all of them.
    wrong: [cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
long long lo=0,hi=0;
for(long long x:a)hi+=x;
auto cnt=[&](long long lim){long long c=0,cur=0;long long l=0;
...
cout<<v[min((long long)v.size(),k)-1]<<"\\n";`)],
  ...
  "array-sum-of-even-values": {
    solution: cpp(`int n;cin>>n;long long s=0;
for(int i=0;i<n;++i){long long x;cin>>x;if(x%2==0)s+=x;}
cout<<s<<"\\n";`),
    // Testing the remainder against one misses every negative even number, whose remainder in C++ is negative rather than one; adding the positions instead of the values answers a different question.
    wrong: [cpp(`int n;cin>>n;long long s=0;
for(int i=0;i<n;++i){long long x;cin>>x;if(x%2!=1)s+=x;}
cout<<s<<"\\n";`), cpp(`int n;cin>>n;long long s=0;
for(int i=0;i<n;++i){long long x;cin>>x;if(i%2==0)s+=x;}
cout<<s<<"\\n";`)],
  },
  "str-longest-word-in-line": {
    solution: cpp(`string w;size_t best=0;
while(cin>>w)if(w.size()>best)best=w.size();
cout<<best<<"\\n";`),
    // Measuring the whole line counts the spaces along with the letters; keeping the last word that was at least as long as the best so far still reports a length, but reading only the first word never looks past it.
    wrong: [cpp(`string line;getline(cin,line);
cout<<line.size()<<"\\n";`), cpp(`string w;cin>>w;
cout<<w.size()<<"\\n";`)],
  },
  "greedy-min-coins-cancel-debts": {
    solution: cpp(`int n;cin>>n;long long s=0;
for(int i=0;i<n;++i){long long x;cin>>x;if(x>0)s+=x;}
cout<<s<<"\\n";`),
    // Adding the magnitudes of every account counts each som twice, once where it leaves and once where it lands; the largest single account says nothing about the accounts beside it.
    wrong: [cpp(`int n;cin>>n;long long s=0;
for(int i=0;i<n;++i){long long x;cin>>x;s+=llabs(x);}
cout<<s<<"\\n";`), cpp(`int n;cin>>n;long long best=0;
...
for(int j=0;j<m;++j){
 for(int i=n-1;i>=0;--i)cout<<a[i][j]<<((i>0)?' ':'\\n');}`),
    // Reading each column from the top down turns the matrix the other way, anticlockwise; transposing alone swaps the axes but never reverses anything.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<vector<long long>>a(n,vector<long long>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
for(int j=m-1;j>=0;--j){
 for(int i=0;i<n;++i)cout<<a[i][j]<<((i+1<n)?' ':'\\n');}`), cpp(`int n,m;cin>>n>>m;vector<vector<long long>>a(n,vector<long long>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
for(int j=0;j<m;++j){
 for(int i=0;i<n;++i)cout<<a[i][j]<<((i+1<n)?' ':'\\n');}`)],
  },
  "sort-count-increasing-triples": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long total=0;
for(int j=0;j<n;++j){
 long long lo=0,hi=0;
 ...
 total+=lo*hi;}
cout<<total<<"\\n";`),
    // Allowing equal values on either side of the middle element admits triples the strict comparison rules out; counting only the triples made of three positions in a row ignores every triple that leaves a gap.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long total=0;
for(int j=0;j<n;++j){
 long long lo=0,hi=0;
...
cout<<total<<"\\n";`)],
  ...
  "dp-max-sum-k-non-adjacent-pairs": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
for(int i=2;i<=n;++i){
 ...
 for(int t=1;t<4;++t){
  ...
  dp[i][t]=best2;}}
cout<<dp[n][3]<<"\\n";`),
    // Taking the three largest values ignores that two of them may stand side by side; picking one element and then the best pair outside it greedily fixes the first choice before the other two are known.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<best<<"\\n";`)],
  ...
  "bit-count-pairs-xor-zero-prefix": {
    solution: cpp(`int n;cin>>n;map<long long,long long>cnt;
cnt[0]=1;long long pre=0,ans=0;
for(int i=0;i<n;++i){long long x;cin>>x;pre^=x;
 ans+=cnt[pre];++cnt[pre];}
cout<<ans<<"\\n";`),
    // Leaving out the empty prefix loses every stretch that starts at the first element; counting only the elements that are zero themselves ignores the longer stretches that cancel out.
    wrong: [cpp(`int n;cin>>n;map<long long,long long>cnt;
long long pre=0,ans=0;
for(int i=0;i<n;++i){long long x;cin>>x;pre^=x;
 ans+=cnt[pre];++cnt[pre];}
cout<<ans<<"\\n";`), cpp(`int n;cin>>n;long long ans=0;
for(int i=0;i<n;++i){long long x;cin>>x;if(x==0)++ans;}
cout<<ans<<"\\n";`)],
  },
  "dp-max-subarray-one-deletion": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
const long long NEG=(long long)-4e18;
...
cout<<best<<"\\n";`),
    // Never dropping anything is the plain largest-stretch problem and misses the gain a single bad element's removal brings; adding the current element on top of a state that already means it was dropped counts it after deleting it.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<best<<"\\n";`)],
  ...
  "tree-count-vertices-at-even-depth": {
    solution: cpp(`int n;cin>>n;vector<vector<int>>g(n);
for(int i=0;i+1<n;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);g[b].push_back(a);}
...
while(!st.empty()){int v=st.back();st.pop_back();
 for(int u:g[v]){
  ...
  st.push_back(u);}}
cout<<max(c0,c1)<<"\\n";`),
    // Splitting the vertices by their index parity has nothing to do with the tree's own shape; rounding the vertex count up assumes the two colours always come out as even as possible, which a star never does.
    wrong: [cpp(`int n;cin>>n;
...
cout<<max(leaves,(long long)n-leaves)<<"\\n";`)],
  ...
  "adv-offline-range-distinct-count": {
    solution: cpp(`int n,q;cin>>n>>q;vector<long long>a(n+1);
for(int i=1;i<=n;++i)cin>>a[i];
...
cout<<out;`),
    // Marking a repeated value at its new position without clearing the old one counts it once for every time it appears; answering the queries in the order they were sorted rather than the order they arrived prints the right numbers against the wrong questions.
    wrong: [cpp(`int n,q;cin>>n>>q;vector<long long>a(n+1);
for(int i=1;i<=n;++i)cin>>a[i];
vector<vector<pair<int,int>>>byR(n+1);
for(int i=0;i<q;++i){int l,r;cin>>l>>r;byR[r].push_back(make_pair(l,i));}
vector<long long>bit(n+2,0);
...
string out;
for(int i=0;i<q;++i){out+=to_string(ans[i]);out+="\\n";}
cout<<out;`), cpp(`int n,q;cin>>n>>q;vector<long long>a(n+1);
for(int i=1;i<=n;++i)cin>>a[i];
vector<vector<pair<int,int>>>byR(n+1);
for(int i=0;i<q;++i){int l,r;cin>>l>>r;byR[r].push_back(make_pair(l,i));}
vector<long long>bit(n+2,0);
...
cout<<out;`)],
  ...
  "num-count-digit-occurrences": {
    solution: cpp(`string s;int d;cin>>s>>d;
long long c=0;
for(char ch:s)if(ch-'0'==d)++c;
cout<<c<<"\\n";`),
    // Peeling digits off with a loop that stops while the number is positive never looks at the single digit of zero; comparing the digit against the character rather than its value never matches.
    wrong: [cpp(`long long n;int d;cin>>n>>d;
long long c=0;
while(n>0){if(n%10==d)++c;n/=10;}
cout<<c<<"\\n";`), cpp(`string s;int d;cin>>s>>d;
long long c=0;
for(char ch:s)if(ch==d)++c;
cout<<c<<"\\n";`)],
  },
  "date-day-of-year": {
    solution: cpp(`int y,m,d;cin>>y>>m>>d;
...
cout<<t<<"\\n";`),
    // Treating every year divisible by four as a leap year adds a day to 1900, which the hundred-year rule excludes; adding the length of the current month as well overshoots by a whole month.
    wrong: [cpp(`int y,m,d;cin>>y>>m>>d;
bool leap=(y%4==0);
int len[12]={31,28,31,30,31,30,31,31,30,31,30,31};
if(leap)len[1]=29;
long long t=d;
for(int i=0;i+1<m;++i)t+=len[i];
cout<<t<<"\\n";`), cpp(`int y,m,d;cin>>y>>m>>d;
bool leap=(y%4==0&&y%100!=0)||(y%400==0);
int len[12]={31,28,31,30,31,30,31,31,30,31,30,31};
if(leap)len[1]=29;
long long t=d;
...
for(int k=1;k<=n;++k){
 ...
 if(ok){cout<<k<<"\\n";return 0;}}
cout<<n<<"\\n";`),
    // Checking only the first repeat of the block leaves the rest of the string unverified; searching from the longest block downwards reports the longest repeating unit rather than the shortest.
    wrong: [cpp(`string s;cin>>s;int n=(int)s.size();
for(int k=1;k<=n;++k){
 if(n%k)continue;
 bool ok=true;
...
cout<<best<<"\\n";`)],
  ...
  "greedy-max-items-within-budget": {
    solution: cpp(`long long n,b;cin>>n>>b;vector<long long>p(n);for(auto&x:p)cin>>x;
sort(p.begin(),p.end());
...
for(long long x:p){
 ...
 spent+=x;++c;}
cout<<c<<"\\n";`),
    // Buying in the order the prices arrive can be stopped by one expensive item while cheaper ones still wait behind it; refusing a purchase that spends the budget exactly rejects a perfectly affordable item.
    wrong: [cpp(`long long n,b;cin>>n>>b;vector<long long>p(n);for(auto&x:p)cin>>x;
long long c=0,spent=0;
for(long long x:p){
 if(spent+x>b)break;
 spent+=x;++c;}
cout<<c<<"\\n";`), cpp(`long long n,b;cin>>n>>b;vector<long long>p(n);for(auto&x:p)cin>>x;
sort(p.begin(),p.end());
long long c=0,spent=0;
for(long long x:p){
 if(spent+x>=b)break;
...
cout<<c<<"\\n";`)],
  ...
  "stack-score-of-brackets": {
    solution: cpp(`string s;cin>>s;vector<long long>st;st.push_back(0);
for(char c:s){
 if(c=='('){st.push_back(0);continue;}
 long long v=st.back();st.pop_back();
 st.back()+=(v==0)?1:2*v;}
cout<<st.back()<<"\\n";`),
    // Counting the pairs and doubling by the depth once at the end mixes the levels of independent sequences together; doubling every closing bracket regardless of what it encloses turns an empty pair into a zero.
    wrong: [cpp(`string s;cin>>s;long long depth=0,best=0,pairs=0;
for(size_t i=0;i<s.size();++i){
 if(s[i]=='(')++depth;
 else{
...
cout<<(pairs*p)<<"\\n";`), cpp(`string s;cin>>s;vector<long long>st;st.push_back(0);
for(char c:s){
 if(c=='('){st.push_back(0);continue;}
 long long v=st.back();st.pop_back();
 st.back()+=2*v;}
cout<<st.back()<<"\\n";`)],
  ...
  "bs-min-max-adjacent-after-inserts": {
    solution: cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
double lo=0,hi=0;
...
printf("%.6f\\n",hi);`),
    // Spreading the new markers evenly over the whole road ignores that a gap can only be split where markers already stand; putting every new marker into the single widest gap never touches the second widest, which then decides the answer.
    wrong: [cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
double total=(double)(a[n-1]-a[0]);
printf("%.6f\\n",total/(double)(n-1+k));`), cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
double widest=0;
for(long long i=1;i<n;++i)widest=max(widest,(double)(a[i]-a[i-1]));
printf("%.6f\\n",widest/(double)(k+1));`)],
  },
  "graph-topo-count-fixed-order": {
    solution: cpp(`int n,m;cin>>n>>m;vector<vector<int>>g(n);vector<int>indeg(n,0);
for(int i=0;i<m;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);++indeg[b];}
vector<int>ready;
...
cout<<((uniq&&done==n)?"YES":"NO")<<"\\n";`),
    // Counting the vertices with no incoming edge only at the start misses a later step where two become ready at once; requiring one fewer edge than vertices is necessary for a unique order but says nothing about how those edges are arranged.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<int>indeg(n,0);
...
while(e>0){
 ...
 A=mul(A,A);e>>=1;}
cout<<R[0][n-1]<<"\\n";`),
    // Stepping the vector forward k times cannot finish when k reaches its limit, and capping the number of steps answers a shorter walk instead; leaving the identity out of the power turns a walk of length zero into an impossible one.
    wrong: [cpp(`long long n,k;cin>>n>>k;const long long M=1000000007;
vector<vector<long long>>A(n,vector<long long>(n,0));
for(long long i=0;i<n;++i)for(long long j=0;j<n;++j)cin>>A[i][j];
...
cout<<out;`),
    // Stopping the climb once the two vertices sit at the same depth skips the stretch above that point, which is where the heaviest edge often is; forgetting the last edge into the meeting vertex leaves one edge of the path unmeasured.
    wrong: [cpp(`int n,q;cin>>n>>q;
vector<vector<pair<int,long long>>>g(n);
for(int i=0;i+1<n;++i){int a,b;long long w;cin>>a>>b>>w;--a;--b;
 g[a].push_back(make_pair(b,w));g[b].push_back(make_pair(a,w));}
...
cout<<out;`)],
  ...
  "adv-count-subarrays-bounded-max": {
    solution: cpp(`long long n,L,R;cin>>n>>L>>R;vector<long long>a(n);for(auto&x:a)cin>>x;
auto atMost=[&](long long lim){long long total=0,run=0;
 ...
 return total;};
cout<<(atMost(R)-atMost(L-1))<<"\\n";`),
    // Counting the stretches whose maximum stays at most R without removing those that never reach L includes every stretch made only of small values; comparing against L rather than the value just below it drops the stretches whose maximum is exactly L.
    wrong: [cpp(`long long n,L,R;cin>>n>>L>>R;vector<long long>a(n);for(auto&x:a)cin>>x;
auto atMost=[&](long long lim){long long total=0,run=0;
 for(long long i=0;i<n;++i){
  if(a[i]<=lim)++run;else run=0;
...
cout<<(atMost(R)-atMost(L))<<"\\n";`)],
  ...
  "array-longest-increasing-prefix": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
int k=1;
while(k<n&&a[k]>a[k-1])++k;
cout<<k<<"\\n";`),
    // Allowing equal neighbours to continue the opening contradicts the strict rise the statement asks for; measuring the longest rising run anywhere ignores that the stretch has to start at the first element.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
int k=1;
while(k<n&&a[k]>=a[k-1])++k;
cout<<k<<"\\n";`), cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<best<<"\\n";`)],
  ...
  "math-arith-progression-sum": {
    solution: cpp(`long long a,d,n;cin>>a>>d>>n;
cout<<(n*a+d*n*(n-1)/2)<<"\\n";`),
    // The last term is a + (n − 1)d rather than a + nd, so stepping one too far inflates the sum; leaving out the halving counts every step twice over.
    wrong: [cpp(`long long a,d,n;cin>>a>>d>>n;
long long last=a+d*n;
cout<<(n*(a+last)/2)<<"\\n";`), cpp(`long long a,d,n;cin>>a>>d>>n;
cout<<(n*a+d*n*(n-1))<<"\\n";`)],
  },
  "dp-min-steps-to-one": {
    solution: cpp(`int n;cin>>n;vector<int>dp(n+1,0);
for(int v=2;v<=n;++v){
 int best=dp[v-1]+1;
 ...
 dp[v]=best;}
cout<<dp[n]<<"\\n";`),
    // Dividing by three whenever it fits, and otherwise by two, fixes each choice before the rest of the path is known; counting only the decrements ignores the two divisions entirely.
    wrong: [cpp(`long long n;cin>>n;long long c=0;
while(n>1){
 if(n%3==0)n/=3;
 else if(n%2==0)n/=2;
 ...
 ++c;}
cout<<c<<"\\n";`), cpp(`long long n;cin>>n;long long c=0;
while(n>1){--n;++c;}
cout<<c<<"\\n";`)],
  },
  "str-min-deletions-sorted": {
    solution: cpp(`string s;cin>>s;int n=(int)s.size();
...
cout<<(n-keep)<<"\\n";`),
    // Counting the places where a letter falls below its neighbour measures how often the order breaks, not how few letters must go to repair it; demanding a strict rise throws away the repeated letters the statement is happy to keep.
    wrong: [cpp(`string s;cin>>s;long long c=0;
for(size_t i=1;i<s.size();++i)if(s[i]<s[i-1])++c;
cout<<c<<"\\n";`), cpp(`string s;cin>>s;int n=(int)s.size();
vector<int>best(26,0);
...
cout<<c<<"\\n";`),
    // Swapping the two roles looks for the largest in the row and the smallest in the column, which is a different kind of cell; insisting that the cell beat every other entry strictly rejects the ties the statement admits.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<vector<long long>>a(n,vector<long long>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
...
if(k%2==1)sum-=2*mn;
cout<<sum<<"\\n";`),
    // Stopping once the negatives are gone leaves the unused flips unspent, but the statement demands exactly k of them and an odd leftover must cost the smallest value twice; adding up the magnitudes assumes every number can be made positive however few flips there are.
    wrong: [cpp(`long long n,k;cin>>n>>k;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
for(long long i=0;i<n&&k>0;++i){
 if(a[i]>=0)break;
...
cout<<sum<<"\\n";`)],
  ...
  "dp-max-profit-two-trades": {
    solution: cpp(`int n;cin>>n;vector<long long>p(n);for(auto&x:p)cin>>x;
const long long NEG=(long long)-4e18;
...
cout<<sell2<<"\\n";`),
    // Taking the single best trade leaves the second one on the table whenever two separate rises beat one; adding up every rise in the prices allows as many trades as there are up days, which is more than two.
    wrong: [cpp(`int n;cin>>n;vector<long long>p(n);for(auto&x:p)cin>>x;
long long mn=(long long)4e18,best=0;
for(long long x:p){mn=min(mn,x);best=max(best,x-mn);}
cout<<best<<"\\n";`), cpp(`int n;cin>>n;vector<long long>p(n);for(auto&x:p)cin>>x;
long long best=0;
for(int i=1;i<n;++i)if(p[i]>p[i-1])best+=p[i]-p[i-1];
cout<<best<<"\\n";`)],
  },
  "graph-cheapest-path-k-stops": {
    solution: cpp(`long long n,m,k;cin>>n>>m>>k;
vector<array<long long,3>>e(m);
...
cout<<((d[n-1]>=INF)?-1:d[n-1])<<"\\n";`),
    // Relaxing the edges in place lets one round chain several flights together, so the hop limit stops being enforced; an ordinary shortest path ignores the limit altogether.
    wrong: [cpp(`long long n,m,k;cin>>n>>m>>k;
vector<array<long long,3>>e(m);
for(long long i=0;i<m;++i){cin>>e[i][0]>>e[i][1]>>e[i][2];--e[i][0];--e[i][1];}
const long long INF=(long long)4e18;
...
cout<<"-1\\n";`),
    // The latest edge time is only the answer when that very edge is the one that finishes the job, and a redundant edge arriving later pushes it past the truth; adding up the times of the edges that mattered answers a spanning-tree question instead of a moment in time.
    wrong: [cpp(`int n,m;cin>>n>>m;
vector<array<long long,3>>e(m);
for(int i=0;i<m;++i)cin>>e[i][1]>>e[i][2]>>e[i][0];
sort(e.begin(),e.end());
...
cout<<((comps==1)?total:-1)<<"\\n";`)],
  ...
  "adv-count-distinct-subsequences": {
    solution: cpp(`string s;cin>>s;const long long M=1000000007;
...
cout<<((total-1)%M+M)%M<<"\\n";`),
    // Doubling at every letter without removing what the previous copy of that letter already produced counts the repeats again; leaving the empty subsequence in the total reports one more than the statement asks for.
    wrong: [cpp(`string s;cin>>s;const long long M=1000000007;
long long total=1;
for(size_t i=0;i<s.size();++i)total=total*2%M;
cout<<((total-1)%M+M)%M<<"\\n";`), cpp(`string s;cin>>s;const long long M=1000000007;
vector<long long>last(26,-1);
long long total=1;
vector<long long>pre;pre.push_back(1);
for(size_t i=0;i<s.size();++i){
...
cout<<total<<"\\n";`)],
  ...
  "array-count-sign-changes": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long c=0;
for(int i=1;i<n;++i)if((a[i]>0)!=(a[i-1]>0))++c;
cout<<c<<"\\n";`),
    // Counting the negative elements says how many there are, not how often the sign turns over; counting only the drops from positive to negative leaves out every crossing in the other direction.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long c=0;
...
cout<<c<<"\\n";`)],
  ...
  "sort-second-smallest-distinct": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
a.erase(unique(a.begin(),a.end()),a.end());
if(a.size()<2){cout<<"NONE\\n";return 0;}
cout<<a[1]<<"\\n";`),
    // Taking the element at position two of the sorted array without removing the repeats returns the smallest value again whenever it occurs twice; comparing the array's own length against two ignores that the repeats were the problem.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
if(n<2){cout<<"NONE\\n";return 0;}
cout<<a[1]<<"\\n";`), cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<a[1]<<"\\n";`)],
  ...
  "math-count-primes-in-range": {
    solution: cpp(`long long L,R;cin>>L>>R;
...
cout<<c<<"\\n";`),
    // Leaving 1 unmarked lets it slip into the count as a prime; starting the crossing-out at twice the prime rather than its square is still correct, but starting it at the prime itself strikes the prime out too.
    wrong: [cpp(`long long L,R;cin>>L>>R;
vector<char>comp(R+1,0);
if(R>=0)comp[0]=1;
for(long long i=2;i*i<=R;++i){
 if(comp[i])continue;
...
cout<<c<<"\\n";`)],
  ...
  "str-longest-word-chain-prefix": {
    solution: cpp(`int n;cin>>n;vector<string>w(n);for(auto&x:w)cin>>x;
string p;cin>>p;
...
cout<<(found?best:"NONE")<<"\\n";`),
    // Looking for the prefix anywhere inside the word accepts one where it starts in the middle; taking the first match in input order rather than the alphabetically smallest depends on how the words happen to be listed.
    wrong: [cpp(`int n;cin>>n;vector<string>w(n);for(auto&x:w)cin>>x;
string p;cin>>p;
string best="";bool found=false;
for(const string&s:w){
 if(s.find(p)==string::npos)continue;
 if(!found||s<best){best=s;found=true;}}
cout<<(found?best:"NONE")<<"\\n";`), cpp(`int n;cin>>n;vector<string>w(n);for(auto&x:w)cin>>x;
string p;cin>>p;
for(const string&s:w){
 if(s.size()<p.size())continue;
 if(s.compare(0,p.size(),p)!=0)continue;
...
cout<<"NONE\\n";`)],
  ...
  "two-pointers-longest-sum-at-most": {
    solution: cpp(`long long n,s;cin>>n>>s;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<best<<"\\n";`),
    // Shrinking while the sum merely reaches s rejects a stretch that fills the bound exactly; restarting the window at the offending element throws away the tail of it that was still usable.
    wrong: [cpp(`long long n,s;cin>>n>>s;vector<long long>a(n);for(auto&x:a)cin>>x;
long long l=0,cur=0,best=0;
for(long long r=0;r<n;++r){
 cur+=a[r];
...
cout<<best<<"\\n";`)],
  ...
  "dp-count-tilings-2xn": {
    solution: cpp(`long long n;cin>>n;const long long M=1000000007;
long long a=1,b=1;
for(long long i=2;i<=n;++i){long long c=(a+b)%M;a=b;b=c;}
cout<<b<<"\\n";`),
    // Counting the tilings as two to the power of the width treats every column as independent, which the flat tiles are not; starting the recurrence one step along shifts every answer to the next term of the sequence.
    wrong: [cpp(`long long n;cin>>n;const long long M=1000000007;
long long r=1;
...
cout<<b<<"\\n";`)],
  ...
  "grid-shortest-with-one-wall-break": {
    solution: cpp(`int n,m;cin>>n>>m;vector<vector<int>>a(n,vector<int>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
...
cout<<best<<"\\n";`),
    // Walking the maze without the wall budget refuses every route that needs the one break the statement grants; treating the walls as ordinary cells lets the route pass through as many of them as it likes.
    wrong: [cpp(`int n,m;cin>>n>>m;vector<vector<int>>a(n,vector<int>(m));
for(int i=0;i<n;++i)for(int j=0;j<m;++j)cin>>a[i][j];
vector<vector<int>>d(n,vector<int>(m,-1));
if(a[0][0]){cout<<"-1\\n";return 0;}
...
cout<<out;`),
    // Writing the new value into the leaf without carrying the change up the tree leaves every range above it answering with the old one; treating the right end as exclusive drops the last element of every range.
    wrong: [cpp(`int n,q;cin>>n>>q;
int sz=1;while(sz<n)sz*=2;
const long long INF=(long long)4e18;
vector<long long>t(2*sz,INF);
...
cout<<out;`)],
  ...
  "adv-min-cost-assign-two-groups": {
    solution: cpp(`long long n;cin>>n;long long m=2*n;
...
cout<<total<<"\\n";`),
    // Choosing the cheaper city for each candidate on its own ignores that each city must take exactly half of them; sorting by the second city's price rather than by what switching saves compares the wrong quantity.
    wrong: [cpp(`long long n;cin>>n;long long m=2*n;
long long total=0;
for(long long i=0;i<m;++i){long long x,y;cin>>x>>y;total+=min(x,y);}
cout<<total<<"\\n";`), cpp(`long long n;cin>>n;long long m=2*n;
...
cout<<total<<"\\n";`)],
  ...
  "adv-longest-repeated-substring-hash": {
    solution: cpp(`string s;cin>>s;long long n=(long long)s.size();
...
cout<<lo<<"\\n";`),
    // Requiring the two occurrences to stand apart rejects the overlapping pair the statement allows; comparing only the neighbouring stretches of each length misses a repeat that sits far away.
    wrong: [cpp(`string s;cin>>s;long long n=(long long)s.size();
const unsigned long long B=131;
vector<unsigned long long>h(n+1,0),pw(n+1,1);
for(long long i=0;i<n;++i){
 h[i+1]=h[i]*B+(unsigned long long)(s[i]-'a'+1);
...
while(lo<hi){long long mid=lo+(hi-lo+1)/2;
 if(ok(mid))lo=mid;else hi=mid-1;}
cout<<lo<<"\\n";`), cpp(`string s;cin>>s;long long n=(long long)s.size();
long long best=0;
...
cout<<best<<"\\n";`)],
  ...
  "num-round-to-nearest-ten": {
    solution: cpp(`long long n;cin>>n;
long long r=n%10;
cout<<((r<5)?(n-r):(n-r+10))<<"\\n";`),
    // Rounding a last digit of exactly 5 downwards contradicts the rule the statement gives; dividing and multiplying by ten always rounds towards zero and never up.
    wrong: [cpp(`long long n;cin>>n;
long long r=n%10;
cout<<((r<=5)?(n-r):(n-r+10))<<"\\n";`), cpp(`long long n;cin>>n;
cout<<(n/10*10)<<"\\n";`)],
  },
  "array-count-distinct-pairs-equal": {
    solution: cpp(`int n;cin>>n;map<long long,long long>cnt;
...
cout<<t<<"\\n";`),
    // Counting each value's occurrences squared counts a position paired with itself and every pair twice over; counting only the equal neighbours misses the pairs standing apart.
    wrong: [cpp(`int n;cin>>n;map<long long,long long>cnt;
for(int i=0;i<n;++i){long long x;cin>>x;++cnt[x];}
long long t=0;
for(map<long long,long long>::iterator it=cnt.begin();it!=cnt.end();++it)
...
cout<<r<<"\\n";`),
    // Leaving the subtractive forms out of the table writes four as IIII and nine hundred as DCCCC; stopping after one use of each value cannot write a number that needs the same symbol twice.
    wrong: [cpp(`int n;cin>>n;
...
cout<<r<<"\\n";`)],
  ...
  "greedy-min-platforms-meetings": {
    solution: cpp(`int n;cin>>n;vector<pair<long long,int>>ev;ev.reserve(2*n);
...
cout<<best<<"\\n";`),
    // Putting the starts ahead of the ends at the same moment treats a handover as an overlap and asks for a room that is already free; the number of meetings is an upper bound rather than the answer.
    wrong: [cpp(`int n;cin>>n;vector<pair<long long,int>>ev;ev.reserve(2*n);
for(int i=0;i<n;++i){long long s,e;cin>>s>>e;
...
cout<<r<<"\\n";`),
    // Raising two to the number of elements assumes every subset reaches a different value, which repeats and dependencies break; counting the distinct input values still confuses a dependent element with an independent one.
    wrong: [cpp(`int n;cin>>n;const long long M=1000000007;
for(int i=0;i<n;++i){unsigned long long x;cin>>x;}
long long r=1;
...
cout<<c<<"\\n";`),
    // Sorting by where each pair begins rather than where it ends lets a long pair be taken first and block everything behind it; allowing the next pair to start exactly where the last one ended ignores that the rise must be strict.
    wrong: [cpp(`int n;cin>>n;vector<pair<long long,long long>>p(n);
...
cout<<c<<"\\n";`)],
  ...
  "graph-count-paths-fixed-length-dag": {
    solution: cpp(`long long n,m,k;cin>>n>>m>>k;const long long M=1000000007;
...
cout<<total<<"\\n";`),
    // Counting only the paths that start at the first vertex ignores that a path may begin anywhere; adding up every length rather than the one asked for counts the shorter paths as well.
    wrong: [cpp(`long long n,m,k;cin>>n>>m>>k;const long long M=1000000007;
vector<vector<int>>g(n);vector<int>indeg(n,0);
for(long long i=0;i<m;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);++indeg[b];}
vector<int>order;order.reserve((size_t)n);vector<int>st;
for(int v=0;v<n;++v)if(indeg[v]==0)st.push_back(v);
...
long long total=0;
for(int v=0;v<n;++v)total=(total+dp[v][k])%M;
cout<<total<<"\\n";`), cpp(`long long n,m,k;cin>>n>>m>>k;const long long M=1000000007;
vector<vector<int>>g(n);vector<int>indeg(n,0);
for(long long i=0;i<m;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);++indeg[b];}
vector<int>order;order.reserve((size_t)n);vector<int>st;
for(int v=0;v<n;++v)if(indeg[v]==0)st.push_back(v);
...
cout<<total<<"\\n";`)],
  ...
  "dp-weighted-interval-schedule": {
    solution: cpp(`int n;cin>>n;vector<array<long long,3>>j(n);
...
cout<<dp[n]<<"\\n";`),
    // Taking the jobs by falling pay and skipping whatever clashes fixes each choice before the rest is known; counting as many jobs as will fit ignores that a single long job can pay more than several short ones.
    wrong: [cpp(`int n;cin>>n;vector<array<long long,3>>j(n);
...
cout<<total<<"\\n";`), cpp(`int n;cin>>n;vector<array<long long,3>>j(n);
for(int i=0;i<n;++i)cin>>j[i][1]>>j[i][0]>>j[i][2];
sort(j.begin(),j.end());
...
cout<<((dp[full]>=INF)?-1:dp[full])<<"\\n";`),
    // Repeatedly grabbing the book that adds the most uncovered topics is the classic greedy for this problem and is not always optimal; counting the books that cover anything new at all ignores that one book can replace several.
    wrong: [cpp(`int n,m;cin>>n>>m;
vector<int>book(n,0);
for(int i=0;i<n;++i){int c;cin>>c;
 for(int t=0;t<c;++t){int x;cin>>x;book[i]|=1<<(x-1);}}
...
cout<<((mask==full)?c:-1)<<"\\n";`)],
  ...
  "adv-count-subarrays-sum-divisible": {
    solution: cpp(`long long n,k;cin>>n>>k;
...
for(long long i=0;i<n;++i){long long x;cin>>x;
 ...
 ans+=cnt[pre];++cnt[pre];}
cout<<ans<<"\\n";`),
    // Counting the elements that divide by k on their own misses every longer stretch that only adds up to a multiple; leaving out the empty prefix loses every stretch that starts at the first element.
    wrong: [cpp(`long long n,k;cin>>n>>k;
long long ans=0;
for(long long i=0;i<n;++i){long long x;cin>>x;
 if(((x%k)+k)%k==0)++ans;}
...
cout<<ans<<"\\n";`)],
  ...
  "array-running-max-differences": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long best=a[0],c=1;
for(int i=1;i<n;++i)if(a[i]>best){best=a[i];++c;}
cout<<c<<"\\n";`),
    // Comparing each element only with the one just before it calls a small rise a record even when a larger value already stood earlier; allowing an equal value to count contradicts the strict rise the statement asks for.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long c=1;
...
cout<<c<<"\\n";`)],
  ...
  "math-count-trailing-zeros-power": {
    solution: cpp(`long long n;cin>>n;long long c=0;
while(n%2==0){n/=2;++c;}
cout<<c<<"\\n";`),
    // Counting the zeros in the whole binary writing counts the ones sitting between the digits as well; halving while the number merely stays above one keeps going past the last even step.
    wrong: [cpp(`long long n;cin>>n;long long c=0;
...
cout<<c<<"\\n";`)],
  ...
  "str-most-frequent-word": {
    solution: cpp(`int n;cin>>n;map<string,long long>cnt;
...
cout<<best<<"\\n";`),
    // Replacing the best word whenever a count merely ties keeps the last one seen instead of the alphabetically smaller; taking the first word to reach the highest count in input order depends on how the words happen to be listed.
    wrong: [cpp(`int n;cin>>n;map<string,long long>cnt;
for(int i=0;i<n;++i){string w;cin>>w;++cnt[w];}
string best;long long bc=-1;
for(map<string,long long>::iterator it=cnt.begin();it!=cnt.end();++it){
 if(it->second>=bc){bc=it->second;best=it->first;}}
cout<<best<<"\\n";`), cpp(`int n;cin>>n;vector<string>w(n);map<string,long long>cnt;
for(int i=0;i<n;++i){cin>>w[i];++cnt[w[i]];}
string best=w[0];long long bc=cnt[w[0]];
for(int i=1;i<n;++i)if(cnt[w[i]]>bc){bc=cnt[w[i]];best=w[i];}
cout<<best<<"\\n";`)],
  },
  "greedy-min-refills-to-reach": {
    solution: cpp(`long long n,d,k;cin>>n>>d>>k;
...
while(pos+k<d){
 ...
 pos=best;++stops;}
cout<<stops<<"\\n";`),
    // Filling up at the first station within range wastes the reach that a farther one would have given; counting how many stations lie on the road says nothing about how many of them must be used.
    wrong: [cpp(`long long n,d,k;cin>>n>>d>>k;
vector<long long>x(n);for(auto&v:x)cin>>v;
long long pos=0,stops=0;long long i=0;
while(pos+k<d){
 if(i>=n||x[i]>pos+k||x[i]<=pos){cout<<"-1\\n";return 0;}
 pos=x[i];++i;++stops;}
cout<<stops<<"\\n";`), cpp(`long long n,d,k;cin>>n>>d>>k;
vector<long long>x(n);for(auto&v:x)cin>>v;
if(k>=d){cout<<"0\\n";return 0;}
cout<<n<<"\\n";`)],
  },
  "two-pointers-count-triangles": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
long long total=0;
for(int k=n-1;k>=2;--k){
...
cout<<total<<"\\n";`),
    // Allowing the two smaller sides to merely equal the largest admits the flat triple the statement rules out; counting only the triples of neighbouring positions in the sorted array ignores every other combination.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
sort(a.begin(),a.end());
long long total=0;
for(int k=n-1;k>=2;--k){
...
cout<<total<<"\\n";`)],
  ...
  "tree-count-leaves-per-depth-max": {
    solution: cpp(`int n;cin>>n;vector<vector<int>>g(n);
for(int i=0;i+1<n;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);g[b].push_back(a);}
...
cout<<best<<"\\n";`),
    // Reporting how many levels the tree has answers a different question than how crowded the busiest one is; counting the neighbours of the root is only the size of level 1 and misses every level below it.
    wrong: [cpp(`int n;cin>>n;vector<vector<int>>g(n);
for(int i=0;i+1<n;++i){int a,b;cin>>a>>b;--a;--b;g[a].push_back(b);g[b].push_back(a);}
vector<int>dep(n,0);vector<char>seen(n,0);
vector<int>q;q.push_back(0);seen[0]=1;
...
cout<<(long long)g[0].size()<<"\\n";`)],
  ...
  "adv-min-adjacent-swaps-to-sort": {
    solution: cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
for(int i=0;i<n;++i){
 ...
 add(r);++done;}
cout<<inv<<"\\n";`),
    // Counting the places where a value drops below its neighbour measures how often the order breaks, not how many swaps repair it; counting the pairs that are merely out of order without insisting on a strict drop charges for equal values that never need swapping.
    wrong: [cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
long long c=0;
for(int i=1;i<n;++i)if(a[i]<a[i-1])++c;
cout<<c<<"\\n";`), cpp(`int n;cin>>n;vector<long long>a(n);for(auto&x:a)cin>>x;
...
cout<<inv<<"\\n";`)],
  ...
  "adv-digit-dp-count-no-repeat": {
    solution: cpp(`auto count=[](long long N){
 if(N<=0)return 0LL;
...
cout<<(count(R)-count(L-1))<<"\\n";`),
    // Counting up to R without removing the values below L answers a range that starts at one; subtracting the count up to L rather than up to the value below it drops the lower bound itself when it qualifies.
    wrong: [cpp(`auto count=[](long long N){
 if(N<=0)return 0LL;
 string s=to_string(N);int L=(int)s.size();
 vector<vector<array<long long,2>>>memo(L+1,vector<array<long long,2>>(11));
 vector<vector<array<char,2>>>vis(L+1,vector<array<char,2>>(11));
