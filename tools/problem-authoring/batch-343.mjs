...
P.push({
  ...
  en: "Sum from l to r",
  statementUz: "Sizga ikkita l va r butun soni berilgan. l dan r gacha bo‘lgan barcha butun sonlar yig‘indisini toping; ikkala chet ham yig‘indiga kiradi. Oraliqda 10^18 tagacha son bo‘lishi mumkin, shuning uchun ularni birma-bir qo‘shib bo‘lmaydi, yig‘indining o‘zi esa chetlarga ehtiyot bo‘lmasa 64-bitli turdan ham oshib ketadi. l va r manfiy bo‘lishi ham mumkin.",
  statementEn: "You are given two integers l and r. Find the sum of every integer from l to r, with both ends included in the sum. The range may hold up to 10^18 numbers, so they cannot be added one at a time, and the sum itself reaches far beyond a 64-bit type unless the ends are handled with care. Both l and r may be negative.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va q ta so‘rov berilgan. Har bir so‘rovda x qiymati beriladi va massivda x dan qat'iy kichik nechta element borligi so‘raladi. Solishtirish qat'iy, ya'ni x ga teng element sanalmaydi; so‘rov massivdagi qiymatlar oralig‘idan butunlay tashqaridagi sonni ham nomlashi mumkin.",
  statementEn: "You are given an array of n integers and q queries. Each query gives a value x and asks how many elements of the array are strictly less than x. The comparison is strict, so an element equal to x is not counted, and a query may name a value lying outside the array's range entirely.",
...
P.push({
  ...
  statementUz: "Sizga bosh lotin harflaridan iborat s satri va k soni berilgan. Ko‘pi bilan k ta harfni istalgan boshqa harfga almashtirish mumkin. Shu almashtirishlardan keyin hosil bo‘lishi mumkin bo‘lgan, bir xil harflardan tuzilgan eng uzun uzluksiz blokning uzunligini toping. Almashtirishlarning hammasini ishlatish shart emas va blok allaqachon eng ko‘p uchraydigan harfdan emas, ixtiyoriy harfdan iborat bo‘lishi mumkin.",
  statementEn: "You are given a string s of uppercase Latin letters and a number k. At most k characters may be replaced by any other character. Find the length of the longest run of equal characters that can be produced this way. The replacements need not all be used, and the run may consist of any letter, not only the one that is already commonest.",
...
P.push({
  ...
  statementUz: "Sizga 0 va 1 lardan iborat r × c jadval va q ta so‘rov berilgan. Har bir so‘rovda to‘rtburchakning chap yuqori va o‘ng pastki burchaklari beriladi va o‘sha to‘rtburchak butunlay birlardan iboratmi, degan savolga javob so‘raladi. Ikkala burchak ham to‘rtburchakka kiradi, to‘rtburchak bitta katakdan iborat bo‘lishi mumkin va jadval so‘rovlar orasida o‘zgarmaydi.",
  statementEn: "You are given an r × c grid of 0s and 1s and q queries. Each query gives the top-left and bottom-right corners of a rectangle and asks whether that rectangle consists entirely of ones. Both corners belong to the rectangle, a rectangle may be a single cell, and the grid never changes between queries.",
...
P.push({
  ...
  statementUz: "Sizga 1-uchida ildizlangan, n ta uchdan iborat daraxt berilgan. Ildizdan har bir uchgacha bo‘lgan masofalar yig‘indisini toping; masofa qirralar soni bilan o‘lchanadi va ildizning o‘zigacha masofa 0 ga teng. Ildizdan boshqa har bir uch kamida 1 qo‘shadi va chuqur daraxtda jami 32-bitli turdan oshib ketadi.",
  statementEn: "You are given a tree with n nodes rooted at node 1. Find the total of the distances from the root to every node, where distance is measured in edges and the distance to the root itself is 0. Every node other than the root contributes at least 1, and on a deep tree the total runs beyond a 32-bit type.",
