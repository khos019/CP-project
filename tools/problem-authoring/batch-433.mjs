...
P.push({
  ...
  statementUz: "Sizga musbat n butun soni berilgan. Uning barcha musbat bo‘luvchilarining yig‘indisini toping; bunda 1 ham, n ning o‘zi ham hisobga olinadi. n katta bo‘lgani uchun 1 dan n gacha aylanib chiqish yaramaydi: kvadrat ildizigacha yurib, har bir topilgan d bo‘luvchi bilan birga n / d juftini ham qo‘shish kifoya. To‘liq kvadratlarda o‘rtadagi bo‘luvchini ikki marta qo‘shib yubormaslik kerak.",
  statementEn: "You are given a positive integer n. Find the sum of all its positive divisors, counting both 1 and n itself. Since n is large, looping from 1 to n is out of the question: walking up to the square root and adding both a divisor d and its partner n / d is enough. For a perfect square, take care not to add the middle divisor twice.",
...
P.push({
  ...
  statementUz: "Sizga kichik lotin harflaridan iborat s satri berilgan. Uning oxiriga eng kam nechta harf qo‘shsangiz, natija palindromga aylanadi — shuni toping. Harflarni faqat oxiriga qo‘shish mumkin, boshiga yoki o‘rtasiga emas. Bu s ning eng uzun palindrom bo‘lgan oxirgi qismini topishga teng: qolgan bosh qismini teskari tartibda oxiriga yozib chiqish kifoya.",
  statementEn: "You are given a string s of lowercase Latin letters. Find the smallest number of letters that must be appended to its end to turn it into a palindrome. Letters may only be added at the end, never at the front or in the middle. This amounts to finding the longest suffix of s that is itself a palindrome: whatever is left in front of it gets written out backwards at the end.",
...
P.push({
  ...
  statementUz: "Sizga kamaymaydigan tartibda saralangan ikkita massiv berilgan. Birinchisidan bitta, ikkinchisidan bitta element olib yig‘indi hosil qilamiz; barcha mumkin bo‘lgan yig‘indilarni o‘sish tartibida yozsak, ularning k-chisi qanday bo‘lishini toping. Bir xil qiymatli yig‘indilar alohida-alohida sanaladi. Hamma yig‘indini tuzib chiqish juda ko‘p bo‘lgani uchun, eng kichiklarini uyuma yordamida birin-ketin ochish kerak.",
  statementEn: "You are given two arrays sorted in non-decreasing order. Taking one element from each forms a sum; writing every possible sum in increasing order, find what the k-th of them is. Equal sums are listed separately. Building all the sums is far too many, so the smallest ones have to be uncovered one at a time with a heap.",
...
P.push({
  ...
  statementUz: "Sizga n ta manfiy bo‘lmagan sondan iborat massiv berilgan. Uning barcha bo‘sh bo‘lmagan ketma-ket qismlarining yig‘indilarini o‘sish tartibida yozsak, ularning k-chisi qanday bo‘lishini toping. Qismlar soni n(n + 1)/2 ga teng va teng yig‘indilar alohida sanaladi. Hamma yig‘indini tuzib bo‘lmagani uchun javobni ikkilik qidiruv bilan izlab, har bir nomzod uchun undan oshmaydigan qismlarni ikki ko‘rsatkich bilan sanash kerak.",
  statementEn: "You are given an array of n non-negative integers. Writing the sums of all its non-empty consecutive stretches in increasing order, find what the k-th of them is. There are n(n + 1)/2 such stretches and equal sums are listed separately. Listing them all is impossible, so the answer has to be binary searched, counting for each candidate how many stretches stay within it using two pointers.",
