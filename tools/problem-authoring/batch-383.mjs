...
P.push({
  ...
  statementUz: "Sizga ikkita kamaymaydigan tartibda saralangan massiv berilgan. Ikkalasida ham uchraydigan har xil qiymatlar nechtaligini sanang. Qiymat har bir massivda necha marta takrorlanishidan qat'i nazar, javobga bittadan hissa qo‘shadi. Ikkala massiv ham saralangani uchun ularni bir vaqtda, ikki ko‘rsatkich bilan yurib chiqish kifoya.",
  statementEn: "You are given two arrays, each sorted in non-decreasing order. Count how many distinct values appear in both. A value contributes one to the answer however many times it repeats in either array. Since both arrays are sorted, walking them together with two pointers is enough.",
...
P.push({
  ...
  statementUz: "Sizga tomonlari koordinata o‘qlariga parallel ikkita to‘rtburchak berilgan; har biri chap pastki va o‘ng yuqori burchagi bilan aniqlanadi. Ular birgalikda qoplaydigan yuzani toping. Ikkala to‘rtburchak kesishsa, kesishma yuzasi ikki marta sanalmasligi kerak: birlashma yuzasi ikkala yuzaning yig‘indisidan kesishma yuzasini ayirganga teng.",
  statementEn: "You are given two axis-aligned rectangles, each by its bottom-left and top-right corner. Find the area they cover together. If the two overlap, the shared part must not be counted twice: the area of the union is the sum of the two areas minus the area of the overlap.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va k soni berilgan. Barcha elementlarni guruhlarga bo‘ling, shunday-ki har bir guruh ichida eng katta va eng kichik qiymat farqi k dan oshmasin. Kerak bo‘ladigan eng kam guruhlar sonini toping. Guruhlar massivdagi tartibga bog‘liq emas — istalgan elementlarni birga qo‘yish mumkin, shuning uchun avval saralab olish foydali.",
  statementEn: "You are given an array of n integers and a number k. Split every element into groups so that within each group the difference between the largest and smallest value is at most k. Find the fewest groups needed. The groups do not depend on the order in the array — any elements may be put together — so sorting first pays off.",
