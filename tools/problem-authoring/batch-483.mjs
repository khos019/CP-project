...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv berilgan. Shunday (i, j) pozitsiyalar juftligi nechtaligini sanang-ki, i < j bo‘lsin va a_i = a_j tengligi bajarilsin. Bir xil qiymat k marta uchrasa, u k(k − 1)/2 ta juftlik beradi. Barcha juftliklarni birma-bir tekshirish sekin, shuning uchun har bir qiymatning uchrash sonini sanab, formulani qo‘llash kerak.",
  statementEn: "You are given an array of n integers. Count the pairs of positions (i, j) with i < j and a_i = a_j. A value occurring k times contributes k(k − 1)/2 such pairs. Checking every pair one at a time is too slow, so count how often each value occurs and apply the formula.",
...
P.push({
  ...
  statementUz: "Sizga n ta uchi va m ta yo‘naltirilgan qirrasi bo‘lgan siklsiz graf va k soni berilgan. Aynan k ta qirradan o‘tuvchi yo‘llar nechtaligini 10^9 + 7 modul bo‘yicha sanang. Yo‘l ixtiyoriy uchdan boshlanib ixtiyoriy uchda tugashi mumkin. Graf siklsiz bo‘lgani uchun uchlarni topologik tartibda ko‘rib chiqib, har bir uch uchun uzunlik bo‘yicha hisobni oldinga uzatish kifoya.",
  statementEn: "You are given a directed graph with n vertices and m edges and no cycles, together with a number k. Count, modulo 10^9 + 7, the paths that use exactly k edges. A path may start at any vertex and end at any vertex. Since the graph has no cycle, walking the vertices in topological order and pushing the counts forward by length is enough.",
...
P.push({
  ...
  statementUz: "Sizga n ta ish berilgan; i-ish s_i vaqtida boshlanib e_i vaqtida tugaydi va p_i foyda keltiradi. Bir vaqtning o‘zida faqat bitta ish bajariladi, lekin bir ish tugagan lahzada boshqasini boshlash mumkin. Umumiy foydani eng katta qiladigan ishlar to‘plamini tanlang va o‘sha foydani chiqaring. Ishlarni tugash vaqti bo‘yicha tartiblab, har biri uchun undan oldin tugagan oxirgi ishni ikkilik qidiruv bilan topish kerak.",
  statementEn: "You are given n jobs, the i-th running from s_i to e_i and paying p_i. Only one job can run at a time, though a job may start at the very moment another ends. Choose the set of jobs that earns the most and print that total. Sorting the jobs by their end time and binary searching for the last job that finishes in time is the way through.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va k soni berilgan. Ketma-ket turgan shunday bo‘sh bo‘lmagan qismlar sonini toping-ki, ulardagi elementlarning yig‘indisi k ga qoldiqsiz bo‘linsin. Prefiks yig‘indilarning k bo‘yicha qoldiqlarini sanash kifoya: bir xil qoldiq ikki joyda uchrasa, ular orasidagi qism k ga bo‘linadi. Manfiy sonlar bo‘lgani uchun qoldiqni musbat oraliqqa keltirish kerak.",
  statementEn: "You are given an array of n integers and a number k. Count the non-empty stretches of consecutive elements whose sum divides by k with no remainder. Counting the remainders of the prefix sums modulo k settles it: whenever the same remainder appears twice, the stretch between them divides by k. Since negative numbers are allowed, the remainder has to be brought back into the positive range.",
