...
P.push({
  ...
  statementUz: "Do‘konda n ta buyum bor; i-buyumning narxi p_i. Sizda b so‘m pul bor. Umumiy narxi byudjetdan oshmasligi sharti bilan eng ko‘pi bilan nechta buyum sotib olishingiz mumkinligini toping. Har bir buyum bir martadan olinadi va buyumlarning qiymati ahamiyatsiz — faqat soni muhim.",
  statementEn: "A shop has n items, the i-th priced at p_i, and you have a budget of b. Find the largest number of items you can buy so that their total price stays within the budget. Each item may be bought once and their value does not matter — only the count does.",
...
P.push({
  ...
  statementUz: "Sizga n ta uchi va m ta yo‘naltirilgan qirrasi bo‘lgan siklsiz graf berilgan. Uning topologik tartibi yagonami, aniqlang. Topologik tartib deganda har bir qirra oldingi uchdan keyingisiga yo‘naladigan joylashuv tushuniladi. Bitta uchli grafda tartib yagona; qirrasiz grafda esa u faqat yagona uch bo‘lgandagina yagona bo‘ladi.",
  statementEn: "You are given a directed graph with n vertices and m edges and no cycles. Determine whether its topological order is unique. A topological order is an arrangement in which every edge runs from an earlier vertex to a later one. A graph of one vertex has a unique order, and a graph with no edge at all has one only when it holds a single vertex.",
...
P.push({
  ...
  statementUz: "Sizga n ta uchi bo‘lgan yo‘naltirilgan graf qo‘shnilik matritsasi ko‘rinishida berilgan. 1-uchdan n-uchgacha aynan k ta qirradan o‘tuvchi yurishlar nechtaligini 10^9 + 7 modul bo‘yicha sanang. Yurishda uchlar va qirralar takrorlanishi mumkin. Uzunligi 0 bo‘lgan yurish faqat boshi va oxiri ustma-ust tushganda mavjud; k esa juda katta bo‘lishi mumkin.",
  statementEn: "You are given a directed graph with n vertices as an adjacency matrix. Count, modulo 10^9 + 7, the walks from vertex 1 to vertex n that use exactly k edges. A walk may repeat vertices and edges. A walk of length 0 exists only when the start and the end coincide, and k may be enormous.",
...
P.push({
  ...
  statementUz: "Sizga n ta uchi bo‘lgan og‘irlikli daraxt va q ta so‘rov berilgan. Har bir so‘rov ikkita uchni beradi va ular orasidagi yagona yo‘lda eng og‘ir qirraning og‘irligini so‘raydi. So‘rovlar juda ko‘p bo‘lgani uchun har bir yo‘lni bir chetidan ikkinchisigacha yurib chiqib bo‘lmaydi, yo‘l esa daraxt chuqurligicha qirradan o‘tishi mumkin. Ikkala uch bir xil bo‘lsa, javob 0 bo‘ladi.",
  statementEn: "You are given a weighted tree with n vertices and q queries. Each query names two vertices and asks for the weight of the heaviest edge on the single path between them. There are far too many queries to walk each path from one end to the other, and a path may run through as many edges as the tree is deep. When the two vertices coincide the answer is 0.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va ikkita L, R chegarasi berilgan. Ketma-ket turgan shunday bo‘sh bo‘lmagan qismlar sonini toping-ki, ulardagi eng katta element L dan kichik bo‘lmasin va R dan katta bo‘lmasin. Qism bo‘sh bo‘lmasligi va elementlari ketma-ket turishi shart; javob 32-bitli turdan oshib ketadi. Bitta elementning xori o‘sha elementning o‘zi, ya'ni yakka turgan 0 ham shunday qism hisoblanadi.",
  statementEn: "You are given an array of n integers and two bounds L and R. Count the non-empty stretches of consecutive elements whose largest value is not below L and not above R. A stretch must be non-empty and its elements consecutive; the count runs beyond a 32-bit type. The xor of a single element is that element itself, so a lone 0 already forms such a stretch.",
