...
P.push({
  ...
  statementUz: "Sizga n ta uchi va m ta qirrasi bo‘lgan yo‘naltirilmagan graf berilgan. Bir-biriga yeta oladigan uchlar juftliklari sonini toping; juftlik tartibsiz, ya'ni (u, v) va (v, u) bitta juftlik hisoblanadi va uchning o‘zi bilan juftligi sanalmaydi. Birorta qirraga tegmagan uch hech qanday juftlik hosil qilmaydi va jami 32-bitli turdan oshib ketadi.",
  statementEn: "You are given an undirected graph with n vertices and m edges. Count the pairs of vertices that can reach each other. A pair is unordered, so (u, v) and (v, u) are one pair, and a vertex paired with itself does not count. A vertex with no edge at all forms no pair, and the total runs beyond a 32-bit type.",
...
P.push({
  ...
  statementUz: "Sizga r radiusi berilgan. Koordinatalar boshida markazlashgan, radiusi r bo‘lgan aylana ichida yoki uning chegarasida yotgan butun koordinatali nuqtalar sonini toping; nuqta (x, y) shartni x² + y² ≤ r² bo‘lganda qanoatlantiradi. Markazning o‘zi ham, aynan aylana ustida yotgan nuqta ham sanaladi, chunki solishtirish qat'iy emas.",
  statementEn: "You are given a radius r. Count the points with integer coordinates lying inside or on the circle of radius r centred at the origin, that is the points (x, y) with x² + y² ≤ r². The centre itself counts, and a point lying exactly on the circle counts too, since the comparison is not strict.",
...
P.push({
  ...
  statementUz: "Sizga n ta uchdan iborat daraxt berilgan. Uchni olib tashlaganda hosil bo‘ladigan komponentalarning eng kattasi n/2 dan oshmasa, o‘sha uch sentroid deyiladi. Daraxtda nechta sentroid borligini aniqlang. Uchni olib tashlaganda unga tegib turgan qirralar ham yo‘qoladi; bitta uchli daraxtda o‘sha uchning o‘zi yagona sentroid bo‘ladi.",
  statementEn: "You are given a tree with n nodes. A node is a centroid when removing it leaves every remaining component with at most n/2 nodes. Determine how many centroids the tree has. Removing a node also removes the edges touching it, and a tree of one node has that node as its only centroid.",
...
P.push({
  ...
  statementUz: "Sizga uchta satr berilgan. Uchalasida ham bir xil nisbiy tartibda uchraydigan eng uzun umumiy qism ketma-ketlik uzunligini toping. Qism ketma-ketlik belgilari yonma-yon turishi shart emas, lekin ularning tartibi har uchala satrda ham saqlanishi kerak. Qism ketma-ketlik bo‘sh bo‘lishi mumkin, shuning uchun uchala satrda umumiy belgi bo‘lmasa javob 0 bo‘ladi.",
  statementEn: "You are given three strings. Find the length of the longest subsequence that appears in all three in the same relative order. The characters of a subsequence need not be adjacent, but their order has to hold in each of the three strings. A subsequence may be empty, so the answer is 0 when the three strings share no character at all.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va q ta amal berilgan. \"1 i x\" amali i-pozitsiyadagi qiymatni x ga almashtiradi. \"2 l r\" amali esa l dan r gacha bo‘lgan pozitsiyalardagi qiymatlar yig‘indisini so‘raydi; ikkala chet ham kiradi. Ikkala chegara ham oraliqqa kiradi, ikki xil amal aralash tartibda keladi va yig‘indi 32-bitli turdan oshib ketadi.",
  statementEn: "You are given an array of n integers and q operations. Operation \"1 i x\" replaces the value at position i with x. Operation \"2 l r\" asks for the sum of the values from position l to position r, with both ends included. Both ends belong to the range, the two kinds of operation arrive mixed together, and a sum runs beyond a 32-bit type.",
