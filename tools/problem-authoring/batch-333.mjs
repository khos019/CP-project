...
P.push({
  ...
  statementUz: "Sizga n va d butun sonlari berilgan. 1 dan n gacha bo‘lgan sonlar orasida raqamlari yig‘indisi d ga qoldiqsiz bo‘linadiganlari nechta ekanini sanang. Raqamlar yig‘indisi odatdagi o‘nlik yozuvda, boshida nolsiz olinadi. n eng katta chegarasiga yetgani uchun sonlarni birma-bir ro‘yxatlab bo‘lmaydi, javobning o‘zi esa 64-bitli turni talab qiladi.",
  statementEn: "You are given integers n and d. Count the numbers from 1 to n whose digits sum to a multiple of d. The digit sum is taken in the usual decimal writing, without leading zeros. Since n reaches its full bound the numbers cannot be listed one by one, and the count itself needs a 64-bit type.",
...
P.push({
  ...
  statementUz: "Sizga hech qachon o‘zgarmaydigan n ta butun sondan iborat massiv va q ta so‘rov berilgan. Har bir so‘rovda l va r indekslari beriladi va a_l, a_{l+1}, …, a_r orasidagi eng katta qiymat so‘raladi; oraliqning ikkala cheti ham kiradi. Ikkala chegara ham oraliqqa kiradi, oraliq bitta elementdan iborat bo‘lishi mumkin va massiv so‘rovlardan oldin bir marta beriladi.",
  statementEn: "You are given an array of n integers that never changes, and q queries. Each query gives indices l and r and asks for the largest value among a_l, a_{l+1}, …, a_r, with both ends included. Both ends belong to the range, a range may hold a single element, and the array is given once before the queries arrive.",
...
P.push({
  ...
  statementUz: "Tribonachchi ketma-ketligi T(1) = T(2) = 1, T(3) = 2 bilan boshlanadi va n ≥ 4 uchun T(n) = T(n−1) + T(n−2) + T(n−3) qoidasiga bo‘ysunadi. Sizga n berilgan; T(n) ning 10^9 + 7 ga bo‘lgan qoldig‘ini toping. n ko‘rsatkichi ketma-ketlikning qiymatlari sig‘adigan chegaradan ancha uzoqqa ketishi mumkin, shuning uchun har bir natija modul ostida chiqariladi.",
  statementEn: "The Tribonacci sequence starts T(1) = T(2) = 1 and T(3) = 2, and follows T(n) = T(n−1) + T(n−2) + T(n−3) for n ≥ 4. Given n, find T(n) modulo 10^9 + 7. The index n may go far beyond anything the sequence's own values would fit in, so every result is reported under the modulus.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va k soni berilgan. Ichida aynan k xil qiymat uchraydigan uzluksiz qism massivlarni sanang. Qism massiv chetlari bilan aniqlanadi, shuning uchun bir xil qiymatlarni saqlaydigan ikki har xil oraliq alohida sanaladi. Qism massiv bo‘sh bo‘lmasligi shart va uning ichida takrorlangan qiymat baribir bitta har xil qiymat deb sanaladi.",
  statementEn: "You are given an array of n integers and a number k. Count the contiguous subarrays that contain exactly k distinct values. A subarray is identified by its endpoints, so two different ranges holding the same values count separately. A subarray must be non-empty, and a value repeated inside it still counts as one distinct value.",
...
P.push({
  ...
  statementUz: "Sizga n ta uchi va m ta qirrasi bo‘lgan yo‘naltirilmagan graf berilgan. So‘ng qirralar berilgan tartibda birma-bir o‘chiriladi. Har bir o‘chirishdan keyin grafda nechta bog‘langan komponenta qolganini chiqaring; boshlang‘ich holat uchun ham bitta son chiqariladi.",
  statementEn: "You are given an undirected graph with n vertices and m edges. The edges are then removed one at a time in the given order. After each removal print how many connected components the graph has, and print one value for the starting state as well.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va q ta so‘rov berilgan. Har bir so‘rovda l va r indekslari beriladi va o‘sha oraliqdagi eng ko‘p uchraydigan qiymat necha marta uchrashi so‘raladi — qiymatning o‘zi emas, uning chastotasi. Ikkala chegara ham oraliqqa kiradi; bir nechta qiymat eng ko‘p uchrashi bo‘yicha teng bo‘lsa ham, javob o‘sha umumiy son bo‘ladi.",
  statementEn: "You are given an array of n integers and q queries. Each query gives indices l and r and asks how often the commonest value in that range occurs — the frequency, not the value itself. Both ends belong to the range, and when several values tie for the commonest the answer is still that shared count.",
