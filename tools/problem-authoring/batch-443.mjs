...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv berilgan. Shunday (i, j, k) uchliklar sonini toping-ki, i < j < k bo‘lsin va a_i < a_j < a_k tengsizliklari bajarilsin. Solishtirishlar qat'iy, ya'ni teng qiymatlar uchlikni buzadi. Uchala pozitsiya o‘sish tartibida turishi shart, ulardagi qiymatlar esa massivning istalgan joyidan bo‘lishi mumkin.",
  statementEn: "You are given an array of n integers. Count the triples (i, j, k) with i < j < k for which a_i < a_j < a_k holds. The comparisons are strict, so equal values break the triple. The three positions must stand in increasing order, and the values at them may come from anywhere in the array.",
  inputUz: "Birinchi qatorda bitta n butun soni beriladi. Ikkinchi qatorda probel bilan ajratilgan n ta butun son keladi.",
  inputEn: "The first line contains one integer n. The second line contains n integers separated by single spaces.",
...
P.push({
  ...
  statementUz: "Sizga n ta butun sondan iborat massiv va q ta so‘rov berilgan. Har bir so‘rov l dan r gacha bo‘lgan oraliqda nechta har xil qiymat borligini so‘raydi. Javoblarni so‘rovlar kelgan tartibda chiqaring. Ikkala chegara ham oraliqqa kiradi, oraliq bitta elementdan iborat bo‘lishi mumkin va massiv so‘rovlar orasida o‘zgarmaydi.",
  statementEn: "You are given an array of n integers and q queries. Each query asks how many distinct values the range from l to r holds. Print the answers in the order the queries arrived. Both ends belong to the range, a range may hold a single element, and the array never changes between queries.",
